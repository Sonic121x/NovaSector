...
#   1. 用 fetchzip 拉取与 dependencies.sh 对齐的版本 (516.1688)；
...
let
  ...
  byondUnpacked = pkgsi686Linux.stdenv.mkDerivation {
    ...
    src = fetchzip {
      ...
      # 516.1688_byond_linux.zip 解包后（stripRoot=false）的 NAR 哈希。
      hash = "sha256-xsqlyxi9S1xO0cUMKjO+BKbBEw5XWtsmRTzKHCfGLkc=";
      stripRoot = false;
    };
