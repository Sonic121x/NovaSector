ADMIN_VERB(toggle_newbie_guard, R_SERVER, "切换新人软管制", "Toggle the region-based soft restriction on low-playtime players.", ADMIN_CATEGORY_SERVER)
	...
	if(enabling)
		if(!newbie_guard_load_geoip(force = TRUE))
			to_chat(user, span_adminnotice("无法启用：地区数据表不可用（[GLOB.newbie_guard_geo_source]）。本表随仓库发布，正常部署不该缺失——请检查 [NEWBIE_GUARD_GEOIP_PATH] 是否随本次部署一同落地。"), confidential = TRUE)
			return

		var/playtime = input(user, "游戏时长低于多少分钟的玩家纳入管制？", "新人软管制", CONFIG_GET(number/newbie_guard_playtime)) as num|null
		if(isnull(playtime))
			return
		var/survival = input(user, "受管制玩家在本回合存活多少分钟后自动解除？", "新人软管制", CONFIG_GET(number/newbie_guard_survival)) as num|null
		if(isnull(survival))
			return
...
ADMIN_VERB(newbie_guard_exempt, R_ADMIN, "豁免新人软管制", "Permanently exempt a ckey from the newbie guard.", ADMIN_CATEGORY_MAIN)
	var/target_ckey = ckey(input(user, "输入要永久豁免的 ckey：", "新人软管制") as text|null)
	if(!target_ckey)
		return
...
ADMIN_VERB(newbie_guard_reload_geoip, R_SERVER, "重载地区数据表", "Reload the newbie guard GeoIP table from disk.", ADMIN_CATEGORY_SERVER)
	...
	GLOB.newbie_guard_verdict.Cut()
	to_chat(user, span_adminnotice("地区数据表：[GLOB.newbie_guard_geo_source]（[loaded] 段）。"), confidential = TRUE)
