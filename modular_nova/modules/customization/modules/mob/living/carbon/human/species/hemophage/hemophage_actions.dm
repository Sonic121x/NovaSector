...
/datum/action/cooldown/hemophage/toggle_dormant_state/Activate(atom/action_target)
	...
	owner.balloon_alert(owner, "[tumor.is_dormant ? "leaving" : "entering"]休眠状态")

	if(!do_after(owner, 3 SECONDS))
		owner.balloon_alert(owner, "状态变更已取消")
		return
...
/datum/action/cooldown/hemophage/hemokinetic_regen/Activate(atom/action_target)
	...
	if(living_owner.get_blood_volume() <= MINIMUM_VOLUME_FOR_REGEN)
		living_owner.balloon_alert(living_owner, "血液水平过低！")
		return
	...
	if(living_owner.has_status_effect(/datum/status_effect/hemokinetic_regen))
		...
		living_owner.balloon_alert(living_owner, "血动再生已停用！")
	else
...
/datum/action/cooldown/hemophage/hemokinetic_clot/Activate(atom/action_target)
	...
	if(carbon_owner.get_blood_volume() <= MINIMUM_VOLUME_FOR_REGEN)
		carbon_owner.balloon_alert(carbon_owner, "血液水平过低！")
		return
	...
	carbon_owner.balloon_alert(carbon_owner, "没有可凝结的伤口！")
