...
/turf/closed/indestructible/hoteldoor/ui_act(action, list/params, datum/tgui/ui)
	...
	switch(action)
		...
		if("set_color")
			...
			return TRUE
		if("pick_color")
			var/picked = input(ui.user, LANG("turf.4c91992f2a355371", null), LANG("turf.9b391b36e6721fde", null), condo_room.lamp_color || "#ffffff") as color|null
			if(picked)
		...
		if("invite")
			...
			if(who)
				to_chat(ui.user, span_notice(LANG("turf.01ad99253d580607", list(who))))
			else
				to_chat(ui.user, span_warning(LANG("turf.a371998d6f67df5e", list(params["name"]))))
			return TRUE
		if("uninvite")
