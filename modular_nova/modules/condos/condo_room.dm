...
/datum/condo_room/proc/try_enter(mob/user)
	switch(access_for(user))
		...
		if(CONDO_ACCESS_PASSWORD)
			var/tried = tgui_input_text(user, LANG("datum.b0bebb56f1e88567", null), LANG("datum.3ee825ef1a9c33b9", null), encode = FALSE, max_length = 42)
			if(isnull(tried))
				return FALSE
			if(tried == password)
				SScondos.warp_into_room(src, user)
				return TRUE
			to_chat(user, span_warning(LANG("datum.b6db61e5ec08b2bf", null)))
			return FALSE
		if(CONDO_ACCESS_DENIED)
			to_chat(user, span_warning(LANG("datum.6b2e16a09d524561", list(display_name))))
	return FALSE
