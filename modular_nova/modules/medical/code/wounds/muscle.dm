...
/datum/wound/muscle/wound_injury(datum/wound/old_wound = null, attack_direction)
	...
	if(held_item && (disabling || prob(30 * severity)))
		...
		if(held_item && victim.dropItemToGround(held_item))
			victim.visible_message(span_danger(LANG("datum.66ca3db39837bc66", list(victim, held_item))), \
			span_warning(LANG("datum.5d9af63c27e36520", list(parse_zone(limb.body_zone), held_item))), vision_distance=COMBAT_MESSAGE_RANGE)
...
/datum/wound/muscle/handle_process()
	...
	if(regen_ticks_current > regen_ticks_needed)
		if(!victim || !limb)
			qdel(src)
			return
		to_chat(victim, span_green(LANG("datum.32ba93523a2e14ea", list(parse_zone(limb.body_zone)))))
		remove_wound()
...
/datum/wound/muscle/proc/attack_with_hurt_hand(mob/M, atom/target, proximity)
	...
	if(prob(severity * 15))
		...
		if(prob(70 - 20 * severity))
			to_chat(victim, span_userdanger(LANG("datum.80e4fffeb82da5b2", list(parse_zone(limb.body_zone), target))))
			limb.receive_damage(brute=rand(1,5))
		else
			victim.visible_message(span_danger(LANG("datum.b0d854bb8e368563", list(victim, target, victim.p_their(), parse_zone(limb.body_zone)))), \
			span_userdanger(LANG("datum.90d601442cd9ebf3", list(target, parse_zone(limb.body_zone)))), vision_distance=COMBAT_MESSAGE_RANGE)
