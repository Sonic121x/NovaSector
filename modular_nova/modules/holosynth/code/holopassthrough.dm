...
/datum/component/glass_passer/holosynth/phase_through_glass(mob/living/owner, atom/bumpee)
	...
	if(is_phase_blocked(first_step, wumpee, owner))
		ascarbon.balloon_alert(ascarbon, LANG("datum.62d831a3dbaf3612", null))
		return
	...
	if(leash?.owner && get_dist(final_destination, leash.owner) > leash.distance)
		ascarbon.balloon_alert(ascarbon, LANG("datum.f5e75781e8f1dc46", null))
		return
	...
	if(ascarbon.handcuffed || ascarbon.legcuffed)
		ascarbon.balloon_alert(ascarbon, LANG("datum.eae3c3df19373c80", null))
		return
...
/datum/action/innate/holosynth_toggle_phase/Activate()
	...
	to_chat(owner, span_notice(LANG("datum.96031cebe8e7858e", list(passer.auto_phase ? "enabled" : "disabled"))))
