...
/datum/admins/proc/manage_player_ranks()
	...
	var/choice = tgui_alert(usr, "你想要管理哪个等级？", "管理玩家等级", NOVA_PLAYER_RANKS)
...
/datum/admins/proc/manage_player_rank_in_group(group)
	...
	switch(tgui_alert(usr, "你想要做什么？", "管理[group]s", choices))
		if("Add")
			var/name = input(usr, "请输入你想设为[group]的人员的CKEY（不区分大小写）：", "添加一个[group]") as null|text
			if(!name)
				return

			var/player_to_be = ckey(name)
			if(!player_to_be)
				to_chat(usr, span_warning("\"[name]\"不是一个有效的CKEY。"))
				return
		...
		if("Remove")
			var/name = input(usr, "请输入你希望不再担任[group]的人员的CKEY（不区分大小写）：", "移除一个[group]") as null|text
			if(!name)
				return
...
/datum/admins/proc/migrate_player_ranks()
	...
	var/choice = tgui_alert(usr, "你想要迁移哪个等级？", "迁移玩家等级", NOVA_PLAYER_RANKS)
	if(!choice || !(choice in NOVA_PLAYER_RANKS))
		return

	if(tgui_alert(usr, "你确定要将[choice]迁移到基于SQL的系统吗？", "迁移玩家等级", list("Yes", "No")) != "Yes")
		return
