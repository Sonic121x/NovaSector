...
/obj/item/clothing/suit/examine(mob/user)
	...
	if(!only_functional)
		. += span_info("Ctrl + Shift + 左键点击可在功能（套装）模式与非功能（颈部）模式之间切换，以便实现诸如将（非功能性）夹克穿在护甲外面以获得视觉效果。")
...
/obj/item/clothing/suit/click_ctrl_shift(mob/user)
	...
	if((char.get_item_by_slot(ITEM_SLOT_NECK) == src) || (char.get_item_by_slot(ITEM_SLOT_OCLOTHING) == src))
		to_chat(user, span_warning("穿戴时无法调整[src]！"))
	...
	if(slot_flags & ITEM_SLOT_OCLOTHING)
		...
		user.visible_message(span_notice("[user]将[user.p_their()]的[src]调整为非功能使用模式。"), span_notice("你将你的[src]调整为非功能使用模式。"))
	else
		...
		user.visible_message(span_notice("[user]将[user.p_their()]的[src]调整为功能使用模式。"), span_notice("你将你的[src]调整为功能使用模式。"))
