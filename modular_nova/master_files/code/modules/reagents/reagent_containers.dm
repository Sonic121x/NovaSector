/obj/item/reagent_containers/click_alt(mob/living/user)
	...
	var/transfer_amount = tgui_input_list(user, "从此容器每次转移的量：", "[src]", possible_transfer_amounts, amount_per_transfer_from_this)
	if(isnull(transfer_amount))
		return NONE
	amount_per_transfer_from_this = transfer_amount
	to_chat(user, span_notice("[src]的转移量现在是[amount_per_transfer_from_this] unit\s 。"))
	return CLICK_ACTION_SUCCESS
