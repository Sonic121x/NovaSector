 ...
 *
 * ⚠️ 2026-07-31 更正：上面这个诊断**不是主因**。实测 `Multiver` 早就在术语表里
 * （→ 多功能解毒剂），目录里照样有 7 种叫法。真因是 `i18n-mt.ts` 的 `sourceTermPattern`
 * **大小写敏感**，而 obj 的 name/desc 按 SS13 惯例全小写（`multiver bottle`），术语表收的是
 * datum 的大写显示名（`Multiver`）→ 小写那半永远匹配不上。改成大小写不敏感后，`terms`
 * 检出量 1314 → 4456。**所以先确认匹配机制，再谈播种覆盖率。**
 * 本脚本仍有价值（收录尚未进表的规范名），但它不是那类 bug 的解药。
 *
 * 通解：把规范名喂给术语表。之后
