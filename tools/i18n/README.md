...
- **单词专用层（`labels.rs` 的 `SINGLE_WORD_TYPE_VAR_RULES`）**：投票/恶意 AI 模块/化学反应/设计/材料/
  试剂的 name **只收单词值**。**注**：这条规则的原始理由（「多词 name 由 P1 就地翻好、单词过不了 P1 的
  多词门槛」）在 2026-08-20 之后已不成立 —— 负载不再被就地改写、P1 的多词门槛也撤了，单词值同样
  进 overlay。规则本身现在是**冗余但无害**的（`tgui.json` 只增不减，撤登记也不会让已有条目消失），
  保留是因为它同时还补 TSX 扫描漏掉的标签。`extract` 里配套一道守门：**单词键的译文只许沿用其它命名空间的既有
