...
/datum/status_effect/stop_drop_roll/on_apply()
	...
	if(!owner.Knockdown(tick_interval * 2, ignore_canstun = TRUE) || owner.body_position != LYING_DOWN)
		to_chat(owner, span_warning("你试图停下、卧倒、翻滚——但你无法趴到地上！"))
		return FALSE
...
/datum/status_effect/stop_drop_roll/proc/start_rolling()
	owner.visible_message(
		span_danger("[owner] 在地板上翻滚，试图扑灭[owner.p_them()]自己身上的火！"),
		span_notice("你停下、卧倒、翻滚！"),
	)
...
/datum/status_effect/stop_drop_roll/hallucinating/start_rolling()
	owner.visible_message(
		span_danger("[owner] 开始在地板上翻滚，胡乱挥舞着手脚！"),
		span_notice("你停下、卧倒、翻滚！"),
	)
...
/datum/status_effect/stop_drop_roll/hallucinating/stop_rolling_successful()
	...
	owner.visible_message(
		span_danger("[owner] 停止了在地面上的胡乱挥舞。"),
		span_notice("你扑灭了自己身上的火。"),
	)
	qdel(src)
