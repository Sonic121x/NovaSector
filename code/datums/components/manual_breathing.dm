...
/datum/component/manual_breathing/Initialize()
	...
	if(L)
		...
		to_chat(C, span_notice("你突然意识到自己在手动呼吸。"))

/datum/component/manual_breathing/Destroy(force)
	L = null
	STOP_PROCESSING(SSdcs, src)
	to_chat(parent, span_notice("你恢复了自动呼吸。"))
	return ..()
...
/datum/component/manual_breathing/process()
	...
	if(world.time > (last_breath + check_every + grace_period))
		if(!warn_dying)
			to_chat(C, span_userdanger("你开始窒息，你需要[next_text]！"))
			warn_dying = TRUE
		...
		if(!warn_grace)
			to_chat(C, span_danger("你感觉需要[next_text]！"))
			warn_grace = TRUE
