...
/datum/component/usb_port/proc/on_examine(datum/source, mob/user, list/examine_text)
	...
	if (isnull(attached_circuit))
		examine_text += span_notice("前面有一个USB端口。")
	else
		examine_text += span_notice("[attached_circuit.shell || attached_circuit]通过USB端口连接到[parent.p_them()]。")

/datum/component/usb_port/proc/on_examine_shell(datum/source, mob/user, list/examine_text)
	SIGNAL_HANDLER

	examine_text += span_notice("[source.p_They()][source.p_are()]通过USB线缆连接到[parent]。")
