...
/datum/component/deployable/proc/examine(datum/source, mob/user, list/examine_list)
	SIGNAL_HANDLER
	examine_list += span_notice("它可以<b>在手中使用</b>来部署成 [((deployments > 1) && multiple_deployments) ? "[deployments]" : "a"] [deployed_name]。")
