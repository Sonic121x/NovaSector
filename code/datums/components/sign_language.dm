...
/datum/component/sign_language/proc/on_try_speech(mob/living/source, message, ignore_spam, forced)
	...
	if(HAS_MIND_TRAIT(carbon_parent, TRAIT_MIMING))
		to_chat(carbon_parent, span_green(LANG("datum.50ef346091f1d72d", null)))
		return COMPONENT_CANNOT_SPEAK

	switch(check_signables_state())
		if(SIGN_HANDS_FULL) // Full hands
			carbon_parent.visible_message(LANG("datum.664a7379464cc26a", list(carbon_parent.p_their())), visible_message_flags = EMOTE_MESSAGE)
			return COMPONENT_CANNOT_SPEAK

		if(SIGN_HANDS_COMPLETELY_RESTRAINED) // Restrained
			carbon_parent.visible_message(LANG("datum.b60e377bca8a94b5", list(carbon_parent.p_their())), visible_message_flags = EMOTE_MESSAGE)
			return COMPONENT_CANNOT_SPEAK

		// If we're handcuffed, we can still sign, but it's slow
		if(SIGN_SLOWLY_FROM_CUFFS)
			carbon_parent.visible_message(LANG("datum.fbc24274c6715c8e", list(carbon_parent.p_their())), visible_message_flags = EMOTE_MESSAGE)
			return COMPONENT_IGNORE_CAN_SPEAK

		if(SIGN_ARMLESS) // No arms
			to_chat(carbon_parent, span_warning(LANG("datum.c8c9fc3bf2518dec", null)))
			return COMPONENT_CANNOT_SPEAK

		if(SIGN_ARMS_DISABLED) // Arms but they're disabled
			to_chat(carbon_parent, span_warning(LANG("datum.e544294d940a0579", null)))
			return COMPONENT_CANNOT_SPEAK

		if(SIGN_TRAIT_BLOCKED) // Hands blocked or emote mute
			to_chat(carbon_parent, span_warning(LANG("datum.b188e4514f649598", null)))
			return COMPONENT_CANNOT_SPEAK
...
/datum/component/sign_language/proc/can_cast_spell(mob/living/carbon/source, datum/action/cooldown/spell/spell, feedback)
	...
	if(spell.invocation_type == INVOCATION_EMOTE) // Mime spells are not cast with signs
		...
		if(feedback)
			to_chat(carbon_parent, span_warning(LANG("datum.a6e21b5508b46c29", list(spell))))
...
/datum/component/sign_language/proc/emote_tone(mob/living/carbon/carbon_parent, emote_tone)
	...
	switch(emote_tone)
		if(TONE_INQUISITIVE)
			carbon_parent.visible_message(span_bold(LANG("datum.fd7e8b96ce42e90a", list(carbon_parent.p_their()))), visible_message_flags = EMOTE_MESSAGE|BLOCK_SELF_HIGHLIGHT_MESSAGE)
		if(TONE_EMPHATIC)
			carbon_parent.visible_message(span_bold(LANG("datum.b86b7043da7d539a", list(carbon_parent.p_their()))), visible_message_flags = EMOTE_MESSAGE|BLOCK_SELF_HIGHLIGHT_MESSAGE)
		if(TONE_INQUISITIVE_EMPHATIC)
			carbon_parent.visible_message(span_bold(LANG("datum.ba6ea2977f288c41", null)), visible_message_flags = EMOTE_MESSAGE|BLOCK_SELF_HIGHLIGHT_MESSAGE)
