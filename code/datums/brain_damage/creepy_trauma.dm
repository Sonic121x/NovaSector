...
/datum/brain_trauma/special/obsessed/proc/do_something_nervous()
	...
	switch(rand(1, 10))
		if(1 to 4)
			...
			to_chat(owner, span_warning(LANG("datum.0fd3596fd69107e5", null)))
		if(5 to 8)
			INVOKE_ASYNC(owner, TYPE_PROC_REF(/mob, emote), "cough")
			to_chat(owner, span_warning(LANG("datum.1ec72c4434e3e332", null)))
		if(9)
			INVOKE_ASYNC(owner, TYPE_PROC_REF(/mob, emote), "laugh")
			to_chat(owner, span_warning(LANG("datum.d49a67633a430fda", null)))
		if(10)
			INVOKE_ASYNC(owner, TYPE_PROC_REF(/mob, emote), "blink")
			owner.adjust_eye_blur_up_to(10 SECONDS, 20 SECONDS)
			to_chat(owner, span_warning(LANG("datum.e869ffdd0aa4410f", null)))
