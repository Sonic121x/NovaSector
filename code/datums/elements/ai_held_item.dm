...
/datum/element/ai_held_item/proc/on_click(mob/living/source, mob/living/user)
	...
	source.visible_message(span_danger("[source] 将[carried_item]丢在了[user]的脚边！"))
...
/datum/element/ai_held_item/proc/on_examined(mob/living/source, mob/user, list/examine_text)
	...
	if (!carried_item)
		return
	examine_text += span_notice("[source.p_They()] [source.p_are()] 携带着[carried_item.examine_title(user)]。")
...
/datum/element/ai_held_item/proc/on_death(mob/living/ol_yeller)
	...
	ol_yeller.visible_message(span_danger("[ol_yeller] 在[ol_yeller.p_they()]死亡[ol_yeller.p_s()]时丢下了[carried_item]。"))
