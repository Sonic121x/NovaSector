	...
	switch(stage)
		if(2)
			...
			if(SPT_PROB(0.5, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.3b633447ee414575", null)))
		if(3)
			if(SPT_PROB(2.5, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.0463c20cf445ee2f", null)))
			else if(SPT_PROB(2.5, seconds_per_tick))
				affected_mob.emote("gasp")
			else if(SPT_PROB(5, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.4d5bd5060b6fadfc", null)))
		if(4)
			if(SPT_PROB(10, seconds_per_tick))
				if(prob(50))
					...
					affected_mob.emote("gasp")
					to_chat(affected_mob, span_danger(LANG("datum.3e939a68a6ca8379", null)))
		if(5)
			to_chat(affected_mob, span_danger(LANG("datum.4bf5dc2b985185c4", null)))
