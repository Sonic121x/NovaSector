	...
	switch(stage)
		...
		if(3)
			if(SPT_PROB(1, seconds_per_tick) && !IS_UNCONSCIOUS_OR_CRIT(affected_mob))
				to_chat(affected_mob, span_warning(LANG("datum.76b56d56ced78363", null)))
			else if(SPT_PROB(1, seconds_per_tick))
				affected_mob.emote("burp")
		if(4)
			if(SPT_PROB(1, seconds_per_tick) && !IS_UNCONSCIOUS_OR_CRIT(affected_mob))
				to_chat(affected_mob, span_warning(LANG("datum.b6e285a4f935f547", null)))
			else if(SPT_PROB(1, seconds_per_tick))
				tritium_burp()
		if(5)
			if(SPT_PROB(1, seconds_per_tick) && !IS_UNCONSCIOUS_OR_CRIT(affected_mob))
				to_chat(affected_mob, span_warning(LANG("datum.7aa1bf5033815e92", null)))
			else if(SPT_PROB(1, seconds_per_tick))
...
/datum/disease/gastritium/proc/tritium_burp(hot_chance = FALSE)
	...
	if(hot_chance && prob(tritium_burp_hot_chance))
		...
		if(!IS_UNCONSCIOUS_OR_CRIT(affected_mob))
			to_chat(affected_mob, span_warning(LANG("datum.cad00aab79e1c90c", null)))
	affected_mob.visible_message(LANG("datum.94a8c219c64256fd", null), visible_message_flags = EMOTE_MESSAGE)
