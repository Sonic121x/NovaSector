	...
	switch(stage)
		...
		if(2)
			if(SPT_PROB(0.8, seconds_per_tick)) // NOVA EDIT CHANGE - ORIGINAL: if(SPT_PROB(0.5, seconds_per_tick))
				to_chat(affected_mob, span_warning(LANG("datum.32fc39568850d411", null)))
				affected_mob.adjust_confusion(6 SECONDS)
			if(SPT_PROB(0.8, seconds_per_tick)) // NOVA EDIT CHANGE - ORIGINAL: if(SPT_PROB(0.5, seconds_per_tick))
				to_chat(affected_mob, span_notice(LANG("datum.dc61f904f20db3ab", null)))
				affected_mob.set_eye_blur_if_lower(10 SECONDS)
		if(3)
			var/need_mob_update = FALSE
			if(SPT_PROB(0.8, seconds_per_tick)) // NOVA EDIT CHANGE - ORIGINAL: if(SPT_PROB(0.5, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.84d02cd6b4525d4d", null)))
				if(prob(45))
			...
			if(SPT_PROB(1, seconds_per_tick)) // NOVA EDIT CHANGE - ORIGINAL: if(SPT_PROB(0.5, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.3b9ee68056b88e8a", null)))
		...
		if(4)
			...
			if(SPT_PROB(1, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.da20c9d198c68d0c", null)))
				if(prob(75))
			...
			if(SPT_PROB(1, seconds_per_tick))
				...
				affected_mob.visible_message(span_warning(LANG("datum.b250dd93af5d065f", list(affected_mob))))
				if(prob(30))
					to_chat(affected_mob, span_danger(LANG("datum.85450d3d649def64", null)))
		...
		if(5)
			switch(rand(1,5)) // NOVA EDIT CHANGE - ORIGINAL: switch(rand(1,2))
				if(1)
					to_chat(affected_mob, span_notice(LANG("datum.7072c9ee58018e68", null)))
					update_stage(1)
				if(2)
					to_chat(affected_mob, span_boldwarning(LANG("datum.1a15cd3b5c072824", null)))
					...
					affected_mob.visible_message(span_warning(LANG("datum.a91a8b1cc8fbca09", list(affected_mob))), span_userdanger(LANG("datum.c9102b9e5f525346", null)))
				...
				if(3)
					to_chat(affected_mob, span_warning(LANG("datum.9c356b27cbdc8171", null)))
					affected_mob.visible_message(span_warning(LANG("datum.70a35d13b553f355", list(affected_mob))), span_userdanger(LANG("datum.5387ba7e28ca51be", null)))
					...
					update_stage(1)
				if(4)
					affected_mob.visible_message(span_warning(LANG("datum.255b4f2b450806dd", list(affected_mob))), span_userdanger(LANG("datum.e983531f75db2ee4", null)))
					affected_mob.gib(DROP_ALL_REMAINS)
					update_stage(1)
				if(5)
					if(!IS_UNCONSCIOUS_OR_CRIT(affected_mob))
						affected_mob.visible_message(span_danger(LANG("datum.32fa6902c51fe3e2", list(affected_mob, affected_mob.p_their(), affected_mob.p_their()))), \
					span_userdanger(LANG("datum.4aff365bf0359ef8", null)))
