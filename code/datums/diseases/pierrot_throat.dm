	...
	switch(stage)
		if(1)
			if(SPT_PROB(5, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.93e9b96a78da020a", null)))
		if(2)
			if(SPT_PROB(5, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.1c4ecc75a84a7020", null)))
		if(3)
			if(SPT_PROB(5, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.984e4b5fc1b67c04", null)))
		if(4)
