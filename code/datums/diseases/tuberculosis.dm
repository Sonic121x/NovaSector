...
/datum/disease/tuberculosis/stage_act(seconds_per_tick) //it begins
	...
	if(SPT_PROB(stage * 2, seconds_per_tick))
		...
		to_chat(affected_mob, span_danger(LANG("datum.500c1472fce9a7e2", null)))

	switch(stage)
		if(2)
			if(SPT_PROB(1, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.279d9ec8a0e3c3a4", null)))
			if(SPT_PROB(2.5, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.be804c2b2877b501", null)))
		if(4)
			...
			if(SPT_PROB(1, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.4709a97c38a9c846", null)))
				need_mob_update += affected_mob.adjust_oxy_loss(5, updating_health = FALSE)
				affected_mob.emote("gasp")
			if(SPT_PROB(5, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.27bba40e72bc3a55", null)))
		...
		if(5)
			...
			if(SPT_PROB(7.5, seconds_per_tick))
				if(ishuman(affected_mob))
					...
					to_chat(affected_mob, span_danger(LANG("datum.f283b9b8b70724fa", null)))
