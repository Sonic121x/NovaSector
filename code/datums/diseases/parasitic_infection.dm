	...
	switch(stage)
		...
		if(2)
			if(SPT_PROB(5, seconds_per_tick))
				if(prob(50))
					to_chat(affected_mob, span_notice("你已经感觉到体重在下降了！"))
				affected_mob.adjust_nutrition(-3)
		if(3)
			if(SPT_PROB(10, seconds_per_tick))
				if(prob(20))
					to_chat(affected_mob, span_notice("你……真的开始感觉到体重下降了。"))
				affected_mob.adjust_nutrition(-6)
		if(4)
			if(SPT_PROB(16, seconds_per_tick))
				if(affected_mob.nutrition >= 100)
					if(prob(10))
						to_chat(affected_mob, span_warning("你感觉身体正在快速减重！"))
					affected_mob.adjust_nutrition(-12)
				else
					to_chat(affected_mob, span_warning("你感觉身体轻多了，真的轻多了！"))
