	...
	switch(stage)
		...
		if(2)
			if(SPT_PROB(5, seconds_per_tick))
				if(prob(50))
					to_chat(affected_mob, span_notice(LANG("datum.11901ebcec697d55", null)))
				affected_mob.adjust_nutrition(-3)
		if(3)
			if(SPT_PROB(10, seconds_per_tick))
				if(prob(20))
					to_chat(affected_mob, span_notice(LANG("datum.7c6b80767a577f76", null)))
				affected_mob.adjust_nutrition(-6)
		if(4)
			if(SPT_PROB(16, seconds_per_tick))
				if(affected_mob.nutrition >= 100)
					if(prob(10))
						to_chat(affected_mob, span_warning(LANG("datum.b80819dd373d9189", null)))
					affected_mob.adjust_nutrition(-12)
				else
					to_chat(affected_mob, span_warning(LANG("datum.f9d05a2d62a77ef3", null)))
