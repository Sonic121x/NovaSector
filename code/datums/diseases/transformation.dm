...
/datum/disease/transformation/proc/replace_banned_player(mob/living/new_mob) // This can run well after the mob has been transferred, so need a handle on the new mob to kill it if needed.
	...
	if(chosen_one)
		to_chat(affected_mob, span_userdanger(LANG("datum.a2252bd960dc95cb", null)))
		...
		to_chat(new_mob, span_userdanger(LANG("datum.a3e0ed8926b29e2d", null)))
	...
	switch(stage)
		if(2)
			if(SPT_PROB(1, seconds_per_tick))
				to_chat(affected_mob, span_notice(LANG("datum.465ed56bac0b41d8", list(pick("arm", "back", "elbow", "head", "leg")))))
		if(3)
			if(SPT_PROB(2, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.d37afbf33c91e366", null)))
	...
	switch(stage)
		...
		if(4)
			...
			if(SPT_PROB(1, seconds_per_tick))
				...
				if(held_item)
					to_chat(affected_mob, span_danger(LANG("datum.94787a4b7fae1957", null)))
