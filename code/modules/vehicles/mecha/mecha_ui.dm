...
/obj/vehicle/sealed/mecha/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("select_module")
			...
			return TRUE
		if("changename")
			var/userinput = tgui_input_text(usr, LANG("obj.d9b96db17517fac2", null), LANG("obj.9a8ade2ecf03d6a9", null), max_length = MAX_NAME_LEN, default = name)
			if(!userinput)
				return
			if(is_ic_filtered(userinput) || is_soft_ic_filtered(userinput))
				tgui_alert(usr, LANG("obj.bbfb99150c282dd1", null))
				return
			if(userinput == format_text(name)) //default mecha names may have improper span artefacts in their name, so we format the name
				to_chat(usr, span_notice(LANG("obj.486142c6eb848115", list(name, userinput))))
				return
