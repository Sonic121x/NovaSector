...
/obj/item/usb_cable/examine(mob/user)
	...
	if (!isnull(attached_circuit))
		. += span_notice(LANG("obj.eaf503a127c62df7", list(attached_circuit.shell || attached_circuit)))
...
/obj/item/usb_cable/pre_attack(atom/target, mob/living/user, list/modifiers, list/attack_modifiers)
	...
	if (prob(1))
		balloon_alert(user, LANG("obj.daf2cb2d690b43ee", null))
		return TRUE
	...
	if (signal_result & COMSIG_USB_CABLE_CONNECTED_TO_CIRCUIT)
		...
		balloon_alert(user, LANG("obj.769acb7e57176900", null))
	...
	if (signal_result & COMSIG_USB_CABLE_ATTACHED)
		...
		balloon_alert(user, LANG("obj.19792e7ea494e1a1", list(connection_description)))
...
/obj/item/usb_cable/suicide_act(mob/living/user)
	user.visible_message(span_suicide(LANG("obj.f0abb3692a664c60", list(user, src, user.p_their(), user.p_theyre()))))
	return OXYLOSS
...
/obj/item/usb_cable/proc/check_in_range()
	...
	if (!IN_GIVEN_RANGE(attached_circuit, src, USB_CABLE_MAX_RANGE))
		balloon_alert_to_viewers(LANG("obj.92779467bc8d6f24", null))
		attached_circuit = null
		return FALSE
