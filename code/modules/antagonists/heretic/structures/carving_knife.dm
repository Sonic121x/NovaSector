...
/obj/item/melee/rune_carver/examine(mob/user)
	...
	. += span_notice(LANG("obj.55607d55038e12c6", list(length(current_runes), max_rune_amt)))
	. += span_info(LANG("obj.9c3d95cca509ba3c", null))
...
/obj/item/melee/rune_carver/proc/try_carve_rune(turf/open/target_turf, mob/user)
	if(drawing)
		target_turf.balloon_alert(user, LANG("obj.ecc823fdda7b5110", null))
		return

	if(locate(/obj/structure/trap/eldritch) in range(1, target_turf))
		target_turf.balloon_alert(user, LANG("obj.144172bdc715c995", null))
		return
	...
	if(length(current_runes) >= max_rune_amt)
		target_turf.balloon_alert(user, LANG("obj.f8b22cf37534f383", null))
		return
...
/obj/item/melee/rune_carver/proc/do_carve_rune(turf/open/target_turf, mob/user)
	...
	target_turf.balloon_alert(user, LANG("obj.5eb827b975e71961", list(picked_choice)))
	user.playsound_local(target_turf, 'sound/items/sheath.ogg', 50, TRUE)
	if(!do_after(user, 5 SECONDS, target = target_turf))
		target_turf.balloon_alert(user, LANG("obj.c67b5d274d6e724b", null))
		return

	target_turf.balloon_alert(user, LANG("obj.b39652ff96b64433", list(picked_choice)))
...
/obj/structure/trap/eldritch/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	if(istype(tool, /obj/item/melee/rune_carver) || HAS_TRAIT(tool, TRAIT_NULLROD_ITEM))
		loc.balloon_alert(user, LANG("obj.5abf070aaee7021b", null))
...
/obj/structure/trap/eldritch/alert/trap_effect(mob/living/victim)
	...
	if(real_owner)
		to_chat(real_owner, span_userdanger(LANG("obj.d8802b127f303429", list(victim.real_name, get_area(src)))))
