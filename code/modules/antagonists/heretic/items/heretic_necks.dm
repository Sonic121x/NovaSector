...
/obj/item/clothing/neck/heretic_focus/crimson_medallion/equipped(mob/living/user, slot)
	...
	to_chat(user, span_alert(LANG("obj.eaa13bd8ef134032", null)))
...
/obj/item/clothing/neck/heretic_focus/crimson_medallion/dropped(mob/living/user)
	...
	if(HAS_TRAIT_FROM(user, TRAIT_MANSUS_TOUCHED, REF(src)))
		to_chat(user, span_notice(LANG("obj.eaeb2777ba118561", null)))
...
/obj/item/clothing/neck/heretic_focus/crimson_medallion/attack_self(mob/living/user, modifiers)
	. = ..()
	to_chat(user, span_danger(LANG("obj.a1ac525bb362ab85", list(src))))
	if(!do_after(user, 1.25 SECONDS, src))
		return
	to_chat(user, span_danger(LANG("obj.8120c7ecf415759b", list(src))))
...
/obj/item/clothing/neck/heretic_focus/crimson_medallion/examine(mob/user)
	...
	var/magic_dude
	if(IS_CULTIST(user))
		. += span_cult_bold(LANG("obj.a8dda4463ad68ef8", null))
		magic_dude = TRUE
	if(IS_HERETIC_OR_MONSTER(user))
		magic_dude = TRUE

	if(magic_dude)
		. += span_red(LANG("obj.3146792a9e17f89b", null))
...
/obj/item/clothing/neck/moon_amulet/examine(mob/user)
	. = ..()
	if(IS_HERETIC(user))
		. += span_notice(LANG("obj.0d1afbf96a68eff1", null))
...
/obj/item/clothing/neck/moon_amulet/proc/channel_amulet(mob/user, atom/target)
	...
	if(!IS_HERETIC_OR_MONSTER(living_user))
		living_user.balloon_alert(living_user, LANG("obj.34e18934aafe08d4", null))
	...
	if(IS_HERETIC_OR_MONSTER(human_target))
		living_user.balloon_alert(living_user, LANG("obj.98b1e53829ea9a95", null))
		return FALSE
	if(human_target.has_status_effect(/datum/status_effect/moon_slept) || human_target.has_status_effect(/datum/status_effect/moon_converted))
		human_target.balloon_alert(living_user, LANG("obj.19307f1581d94c8a", null))
	...
	if(human_target.mob_mood.sanity_level < sanity_threshold)
		human_target.balloon_alert(living_user, LANG("obj.9bfa0589d94f413d", null))
		...
		if(HAS_TRAIT(target, TRAIT_MINDSHIELD))
			human_target.balloon_alert(living_user, LANG("obj.7bd4abf9d631ff49", null))
		...
		human_target.balloon_alert(living_user, LANG("obj.0a493994c82c464d", null))
