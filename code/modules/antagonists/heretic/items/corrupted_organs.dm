...
/obj/item/organ/eyes/corrupt
	...
	pupils_name = span_hypnophrase("被刺穿的真实") //teeny tiny mansus portals, IN YOUR EYEBALLS (known to cause cancer in the state of california)
...
/obj/item/organ/eyes/corrupt/penlight_examine(mob/living/viewer, obj/item/examtool)
	...
	if(!viewer.is_blind() && !IS_HERETIC_OR_MONSTER(viewer))
		to_chat(viewer, span_danger("你的眼球在眼窝中滋滋作响，正遭受着来自异界的能量冲击！"))
		viewer.emote("scream")
	...
	return "[owner.p_Their()]的眼睛[span_hypnophrase(penlight_message)]"
...
/obj/item/organ/stomach/corrupt/proc/on_drank(atom/source, list/reagents, datum/reagents/source_reagents, methods)
	...
	if (!thirst_satiated)
		to_chat(source, span_cult_italic("干渴暂时得到了满足……"))
...
/obj/item/organ/heart/corrupt/hear_beat_noise(mob/living/hearer)
	...
	return "[owner.p_Their()]的心脏发出[span_hypnophrase(beat_noise)]"
...
/obj/item/organ/lungs/corrupt/hear_breath_noise(mob/living/hearer)
	...
	return "[owner.p_Their()]的肺部发出[span_hypnophrase(breath_noise)]"

/// It's full of worms
/obj/item/organ/appendix/corrupt
	name = "腐化阑尾"
	desc = "究竟是何种黑暗的宇宙力量，才会费心去腐化一个阑尾？"
	organ_flags = parent_type::organ_flags | ORGAN_HAZARDOUS
