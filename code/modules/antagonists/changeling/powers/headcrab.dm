...
/datum/action/changeling/headcrab
	name = "破釜沉舟"
	desc = "我们在危急时刻牺牲当前躯体，剧烈膨胀以突破障碍，并重组为头蟹形态。消耗20点化学物质。"
...
/datum/action/changeling/headcrab/sting_action(mob/living/user)
	set waitfor = FALSE
	var/confirm = tgui_alert(user, "我们确定要摧毁自己的身体并创造一个头蛭吗？", "破釜沉舟", list("Yes", "No"))
	if(active || confirm != "Yes")
		return
	active = TRUE
	..()
	user.visible_message(span_boldwarning("[user]的身体开始不自然地搏动和肿胀！"))
...
/datum/action/changeling/headcrab/proc/gore_explosion(mob/living/user)
	...
	user.visible_message(span_boldwarning("[user]的躯体在生物质的剧烈爆炸中破裂！"))
	...
	for(var/mob/living/blinded in view(LAST_RESORT_BLIND_RANGE, user))
		if(blinded == user)
			continue
		blinded.visible_message(span_danger("[blinded]被溅了一身血！"), span_userdanger("你被溅了一身血！"))
		...
		if(ishuman(blinded))
			...
			if(!eyes || blinded_human.is_blind())
				continue
			to_chat(blinded_human, span_userdanger("你被血雨遮蔽了视线！"))
		...
		if(issilicon(blinded))
			...
			to_chat(blinded_silicon, span_userdanger("你的传感器被血雨遮蔽了！"))
...
/datum/action/changeling/headcrab/proc/spawn_headcrab(datum/mind/stored_mind, turf/spawn_location, list/organs)
	...
	to_chat(crab, span_warning("我们在血肉横飞中从旧躯壳的残骸中破体而出！"))
	active = FALSE
...
/datum/action/changeling/headcrab/proc/escaping_prison(mob/living/user)
	user.visible_message(span_boldwarning("[user]膨胀的形体开始碾碎周围的障碍物！"))
