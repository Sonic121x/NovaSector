...
/obj/machinery/plumbing/buffer/examine(mob/user)
	. = ..()
	. += span_notice(LANG("obj.0183978dbed476bc", list(activation_volume)))
	switch(mode)
		if(AB_UNREADY)
			. += span_notice(LANG("obj.ca1cfbf89bb3cde1", null))
		if(AB_IDLE)
			. += span_notice(LANG("obj.03634d60df85f310", null))
		if(AB_READY)
			. += span_notice(LANG("obj.58435440a57a8ad7", null))
	. += span_notice(LANG("obj.9fd3f0bb8fdc2e3a", list(EXAMINE_HINT("hand"))))
	. += span_notice(LANG("obj.fd64e0d170c308ce", list(EXAMINE_HINT("screwdriver"))))
