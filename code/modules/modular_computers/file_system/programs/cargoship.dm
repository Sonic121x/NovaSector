...
/datum/computer_file/program/shipping/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("resetid")
			payments_acc = null
		if("setsplit")
			var/potential_cut = input("你想向注册卡支付多少金额？","利润百分比 ([round(cut_min*100)]% - [round(cut_max*100)]%)") as num|null
		...
		if("print")
			if(computer.stored_paper <= 0)
				...
				return TRUE
			if(!payments_acc)
				to_chat(usr, span_notice("软件错误：请先设置当前用户。"))
				return TRUE
			...
			to_chat(usr, span_notice("电脑打印出了一张条形码。"))
