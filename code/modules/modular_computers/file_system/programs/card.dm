...
/datum/computer_file/program/card_mod/ui_act(action, params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("PRG_terminate")
			...
			if(minor)
				if(!(modified_id.trim?.type in job_templates))
					to_chat(usr, span_notice("软件错误：你没有降级此卡所需的权限。"))
					return TRUE
		...
		if("PRG_edit")
			...
			if(!new_name)
				to_chat(usr, span_notice("软件错误：ID卡拒绝了新名称，因为它包含禁止使用的字符。"))
				return TRUE
		...
		if("PRG_access")
			...
			if(!modified_id.add_access(list(access_type), try_wildcard))
				to_chat(usr, span_notice("ID错误：ID卡拒绝了你的访问权限修改尝试。"))
		...
		if("PRG_insert_alt_id")
			...
			if(!isidcard(alt_id))
				to_chat(user, span_notice("你必须手持一张ID卡才能将其插入副卡槽。"))
				return TRUE
