...
/obj/item/assembly/control/elevator/emag_act(mob/user, obj/item/card/emag/emag_card)
	...
	balloon_alert_loc.balloon_alert(user, LANG("obj.bd40a80f973c0a5b", null))
	return TRUE
...
/obj/item/assembly/control/elevator/proc/call_elevator(mob/activator)
	...
	var/datum/transport_controller/linear/lift = lift_weakref?.resolve()
	if(!lift)
		loc.balloon_alert(activator, LANG("obj.331b019199991145", null))
		return FALSE
	...
	if(lift.controller_status & CONTROLS_LOCKED)
		loc.balloon_alert(activator, LANG("obj.98536372b9001b53", null))
		return FALSE
	...
	if(prime_lift.z == loc.z)
		...
		loc.balloon_alert(activator, LANG("obj.d2d10e7c5dd532a9", null))
		return TRUE
	...
	if(activator)
		loc.balloon_alert(activator, LANG("obj.7b841736315b8f22", null))

	// Actually try to move the lift. This will sleep.
	if(!lift.move_to_zlevel(loc.z, CALLBACK(src, PROC_REF(check_button))))
		loc.balloon_alert(activator, LANG("obj.de46e5e4ea08b672", null))
		return FALSE
	...
	if(!QDELETED(activator))
		loc.balloon_alert(activator, LANG("obj.ea74a93c4c5d6d00", null))
