...
/proc/to_chat_immediate(
	...
	confidential = FALSE,
	skip_i18n_fallback = FALSE // NOVA EDIT ADDITION - i18n - lets a caller opt a player-authored message (e.g. heard speech) out of the chat landing layer so the player's own words aren't auto-translated
)
	...
	// NOVA EDIT ADDITION START - i18n - 聊天落地层（HTML 切块 + 精确反查 + 模板逆匹配）。config I18N_CHAT_FALLBACK 开；英文服/未开启 no-op。跳过玩家/管理员自己输入的频道（按类型）与显式 skip_i18n_fallback（玩家发言），避免误翻玩家原话。
