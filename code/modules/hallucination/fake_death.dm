...
/datum/hallucination/death/start()
	...
	to_chat(hallucinator, span_deadsay("<b>[hallucinator.real_name]</b>已在<b>[get_area_name(hallucinator)]</b>死亡。"))
	...
	if(prob(50))
		...
		if(who_is_salting)
			...
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(to_chat), hallucinator, span_deadsay("<b>死者：[who_is_salting.name]</b>说道：\"[pick(dead_chat_salt)]\"")), delay)
