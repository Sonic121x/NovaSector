/mob/living/silicon/pai/mob_try_pickup(mob/living/user, instant=FALSE)
	if(!possible_chassis[chassis])
		to_chat(user, span_warning("[src]的当前形态无法被携带！"))
		return FALSE
	return ..()
...
/mob/living/silicon/pai/wabbajack(what_to_randomize, change_flags = WABBAJACK)
	...
	balloon_alert(src, "[holochassis]复合体已启用")
	return TRUE
...
/mob/living/silicon/pai/proc/choose_chassis()
	...
	balloon_alert(src, "[choice]复合体已启用")
	update_resting()
	return TRUE
...
/mob/living/silicon/pai/proc/fold_in(force = FALSE)
	...
	if(!holoform)
		. = fold_out(force)
		return FALSE
	visible_message(span_notice("[src]正在解离！"))
	stop_pulling()
...
/mob/living/silicon/pai/proc/fold_out(force = FALSE)
	if(holochassis_health < 0)
		balloon_alert(src, "发射器修复未完成")
		return FALSE
	if(!can_holo && !force)
		balloon_alert(src, "发射器已禁用")
		return FALSE
	if(holoform)
		. = fold_in(force)
		return
	if(!holochassis_ready)
		balloon_alert(src, "发射器正在回收...")
		return FALSE
	holochassis_ready = FALSE
	addtimer(VARSET_CALLBACK(src, holochassis_ready, TRUE), HOLOCHASSIS_COOLDOWN)
