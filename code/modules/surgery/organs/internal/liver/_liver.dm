...
/obj/item/organ/liver/organ_failure(seconds_per_tick)
	switch(failure_time/LIVER_FAILURE_STAGE_SECONDS)
		if(1)
			to_chat(owner, span_userdanger(LANG("obj.0a50e15c", null)))
		if(2)
			to_chat(owner, span_userdanger(LANG("obj.c0d7db52", null)))
			owner.vomit(VOMIT_CATEGORY_DEFAULT)
		if(3)
			to_chat(owner, span_userdanger(LANG("obj.aef35dc8", null)))
			owner.vomit(VOMIT_CATEGORY_BLOOD)
		if(4)
			to_chat(owner, span_userdanger(LANG("obj.886be641", null)))
			owner.vomit(VOMIT_CATEGORY_BLOOD, distance = rand(1,2))
			owner.emote("Scream")
			owner.AdjustUnconscious(2.5 SECONDS)
		if(5)
			to_chat(owner, span_userdanger(LANG("obj.f049270e", null)))
...
/obj/item/organ/liver/proc/on_owner_examine(datum/source, mob/user, list/examine_list)
	...
	switch(failure_time)
		if(0 to 3 * LIVER_FAILURE_STAGE_SECONDS - 1)
			examine_list += span_notice(LANG("obj.23cb13e9", list(owner)))
		if(3 * LIVER_FAILURE_STAGE_SECONDS to 4 * LIVER_FAILURE_STAGE_SECONDS - 1)
			examine_list += span_notice(LANG("obj.96308b27", list(owner, owner.p_they(), owner.p_are())))
		if(4 * LIVER_FAILURE_STAGE_SECONDS to INFINITY)
			examine_list += span_danger(LANG("obj.c6fa14b6", list(owner, owner.p_They(), owner.p_do(), owner.p_they())))
