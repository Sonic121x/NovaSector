...
/obj/item/organ/liver/examine(mob/user)
	...
	if(HAS_MIND_TRAIT(user, TRAIT_ENTRAILS_READER) || isobserver(user))
		if(HAS_TRAIT(src, TRAIT_LAW_ENFORCEMENT_METABOLISM))
			. += span_info(LANG("obj.06db4f65ea6842e1", null))
		if(HAS_TRAIT(src, TRAIT_CULINARY_METABOLISM))
			. += span_info(LANG("obj.8bc8858de7a58ce7", null))
		if (HAS_TRAIT(src, TRAIT_BARTENDER_METABOLISM))
			. += span_info(LANG("obj.dae9408d3506b734", null))
		if(HAS_TRAIT(src, TRAIT_COMEDY_METABOLISM))
			. += span_info(LANG("obj.81069f22c3fe8871", list(span_clown("honking"))))
		if(HAS_TRAIT(src, TRAIT_MEDICAL_METABOLISM))
			. += span_info(LANG("obj.889287724e098113", null))
		if(HAS_TRAIT(src, TRAIT_ENGINEER_METABOLISM))
			. += span_info(LANG("obj.fa88f1b710777b0c", null))
		if(HAS_TRAIT(src, TRAIT_SCIENTIST_LIVER))
			. += span_info(LANG("obj.a9c2eaf805316db8", null))
		if(HAS_TRAIT(src, TRAIT_MAINTENANCE_METABOLISM))
			. += span_info(LANG("obj.5cf3450cc1f91022", null))
		if(HAS_TRAIT(src, TRAIT_CORONER_METABOLISM))
			. += span_info(LANG("obj.139fa31e9cc62872", null))
		if(HAS_TRAIT(src, TRAIT_HUMAN_AI_METABOLISM))
			. += span_info(LANG("obj.491db2819162ffc6", null))

		// royal trumps pretender royal
		if(HAS_TRAIT(src, TRAIT_ROYAL_METABOLISM))
			. += span_info(LANG("obj.b99d7a5ff8af7288", null))
		else if(HAS_TRAIT(src, TRAIT_PRETENDER_ROYAL_METABOLISM))
			. += span_info(LANG("obj.11c26e29c9d087ad", null))
...
/obj/item/organ/liver/organ_failure(seconds_per_tick)
	switch(failure_time/LIVER_FAILURE_STAGE_SECONDS)
		if(1)
			to_chat(owner, span_userdanger(LANG("obj.0a50e15cb3c89c14", null)))
		if(2)
			to_chat(owner, span_userdanger(LANG("obj.c0d7db5290fb4c1d", null)))
			owner.vomit(VOMIT_CATEGORY_DEFAULT)
		if(3)
			to_chat(owner, span_userdanger(LANG("obj.aef35dc8bd5979ff", null)))
			owner.vomit(VOMIT_CATEGORY_BLOOD)
		if(4)
			to_chat(owner, span_userdanger(LANG("obj.886be6411015cdcb", null)))
			owner.vomit(VOMIT_CATEGORY_BLOOD, distance = rand(1,2))
			owner.emote("Scream")
			owner.AdjustUnconscious(2.5 SECONDS)
		if(5)
			to_chat(owner, span_userdanger(LANG("obj.f049270e17cc5c23", null)))
...
/obj/item/organ/liver/proc/on_owner_examine(datum/source, mob/user, list/examine_list)
	...
	switch(failure_time)
		if(0 to 3 * LIVER_FAILURE_STAGE_SECONDS - 1)
			examine_list += span_notice(LANG("obj.23cb13e96a6e6ca6", list(owner)))
		if(3 * LIVER_FAILURE_STAGE_SECONDS to 4 * LIVER_FAILURE_STAGE_SECONDS - 1)
			examine_list += span_notice(LANG("obj.96308b2735182ad6", list(owner, owner.p_they(), owner.p_are())))
		if(4 * LIVER_FAILURE_STAGE_SECONDS to INFINITY)
			examine_list += span_danger(LANG("obj.c6fa14b6e0ad9ef4", list(owner, owner.p_They(), owner.p_do(), owner.p_they())))
