...
/datum/action/cooldown/mob_cooldown/expel_gas/Activate(atom/target)
	...
	var/picked_gas = tgui_input_list(owner, LANG("datum.f55c67bbee9de24b", null), LANG("datum.29e9f1c60eb68924", null), gas_selection)
	StartCooldown()
	if(picked_gas == "None")
		stop_gas()
		return
	...
	if(isguardian(owner))
		...
		if(!guardian_owner.is_deployed())
			to_chat(owner, span_warning(LANG("datum.2e4f2470bfc84937", null)))
			return

	to_chat(owner, span_bolddanger(LANG("datum.85fc1cbd2df1877d", list(picked_gas))))
...
/datum/action/cooldown/mob_cooldown/expel_gas/proc/stop_gas()
	...
	if (!isnull(active_gas))
		to_chat(src, span_notice(LANG("datum.0d8ce6036789e2f2", null)))
	active_gas = null
	QDEL_NULL(mob_smoke)
