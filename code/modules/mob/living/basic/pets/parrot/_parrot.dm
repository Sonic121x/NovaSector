...
/mob/living/basic/parrot/proc/perch_on_human(mob/living/carbon/human/target)
	...
	to_chat(src, span_notice("你坐在[target]的肩膀上。"))
	toggle_perched(perched = TRUE)
...
/mob/living/basic/parrot/proc/steal_from_ground(obj/item/target)
	if(!isnull(held_item))
		balloon_alert(src, "已经拿着东西了！")
		return FALSE

	if(target.w_class > WEIGHT_CLASS_SMALL)
		balloon_alert(src, "太大了拿不起来！")
		return FALSE

	pick_up_item(target)
	visible_message(
		span_notice("[src]抓住了[held_item]！"),
		span_notice("你抓住了[held_item]！"),
		span_hear("你听到翅膀疯狂拍打的声音。"),
	)
	return TRUE
...
/mob/living/basic/parrot/proc/steal_from_mob(mob/living/carbon/victim)
	...
	for(var/obj/item/stealable in victim.held_items)
		...
		visible_message(
			span_notice("[src]从[victim]手中抢走了[held_item]！"),
			span_notice("你从[victim]手中夺走了[held_item]！"),
			span_hear("你听到翅膀疯狂拍打的声音。"),
		)
...
/mob/living/basic/parrot/proc/consume_cracker(obj/item/thing)
	to_chat(src, span_notice("[src] 急切地吞食了 \the [thing]。"))
...
/mob/living/basic/parrot/proc/drop_held_item(gently = TRUE)
	...
	if(!gently && isgrenade(held_item))
		...
		balloon_alert(src, "炸弹投下！") // you'll likely die too so we can get away with the `!` here
		bomb.forceMove(drop_location())
		bomb.detonate()
		return

	balloon_alert(src, "丢下了物品")
	held_item.forceMove(drop_location())
