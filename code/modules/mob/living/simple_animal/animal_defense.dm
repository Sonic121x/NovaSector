/mob/living/simple_animal/attack_hand(mob/living/carbon/human/user, list/modifiers)
	...
	if(!user.combat_mode)
		...
		visible_message(span_notice("[user] [response_help_continuous] [src]。"), \
						span_notice("[user] [response_help_continuous] 你。"), null, null, user)
		to_chat(user, span_notice("你 [response_help_simple] [src]。"))
		...
		visible_message(span_danger("[user][response_harm_continuous][src]！"),\
						span_userdanger("[user][response_harm_continuous]你！"), null, COMBAT_MESSAGE_RANGE, user)
		to_chat(user, span_danger("你[response_harm_simple][src]！"))
...
/mob/living/simple_animal/get_shoving_message(mob/living/shover, obj/item/weapon, shove_flags)
	...
	shover.visible_message(
		...
		span_danger("你[response_disarm_simple] [src][moved ? ", pushing [p_them()]" : ""]！"),
	...
	if (!user.combat_mode)
		if (health > 0)
			visible_message(span_notice("[user.name][response_help_continuous][src]。"), \
							span_notice("[user.name][response_help_continuous]你。"), null, COMBAT_MESSAGE_RANGE, user)
			to_chat(user, span_notice("你[response_help_simple][src]。"))
			playsound(loc, 'sound/items/weapons/thudswoosh.ogg', 50, TRUE, -1)
	...
	if(..()) //if harm or disarm intent.
		if(LAZYACCESS(modifiers, RIGHT_CLICK))
			...
			visible_message(span_danger("[user] [response_disarm_continuous] [name]！"), \
							span_userdanger("[user] [response_disarm_continuous] 你！"), null, COMBAT_MESSAGE_RANGE, user)
