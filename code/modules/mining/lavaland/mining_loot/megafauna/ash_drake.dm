...
/obj/item/drake_remains
	name = "飞龙残骸"
	desc = "一条巨龙的遗骸。它仍在噼啪作响，散发着热量，并带有明显的硫磺气味。"
...
/obj/item/melee/ghost_sword
	name = "\improper 灵体之刃"
	desc = "一把生锈且钝化的刀刃。看起来它造成不了多少伤害。它微弱地发着光。"
...
/obj/item/melee/ghost_sword/attack_self(mob/user)
	if(!COOLDOWN_FINISHED(src, summon_cooldown))
		to_chat(user, span_warning("你刚刚才请求过援助。你不想惹恼那些灵魂！"))
		return

	COOLDOWN_START(src, summon_cooldown, 60 SECONDS)
	to_chat(user, span_notice("你呼唤援助，试图将灵魂召唤到身边。"))
	notify_ghosts(
...
/obj/item/melee/ghost_sword/attack(mob/living/target, mob/living/carbon/human/user)
	...
	user.visible_message(span_danger("[user] 以 [ghost_counter] 个复仇之魂的力量发起攻击！"))
	. = ..()
...
/obj/item/melee/ghost_sword/hit_reaction(mob/living/carbon/human/owner, atom/movable/hitby, attack_text = "the attack", final_block_chance = 0, damage = 0, attack_type = MELEE_ATTACK, damage_type = BRUTE)
	...
	owner.visible_message(span_danger("[owner] 被一圈 [ghost_counter] 个鬼魂保护着！"))
	return ..()
...
/obj/item/dragons_blood/attack_self(mob/living/carbon/human/user)
	...
	switch(random)
		if(1)
			to_chat(user, span_danger("你的外貌变成了一个非常小的人形灰烬龙！你只是获得了他们能力的一小部分，看起来像个怪胎。"))
...
/obj/item/lava_staff/ranged_interact_with_atom(atom/interacting_with, mob/living/user, list/modifiers)
	...
	if(islava(target_turf))
		...
		user.visible_message(span_danger("[user] 把 \the [old_name] 变成了 [reset_string]！"))
	...
	user.visible_message(span_danger("[user] 将 [src] 指向了 [target_turf]！"))
