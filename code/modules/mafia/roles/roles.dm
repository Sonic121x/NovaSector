...
/datum/mafia_role/proc/greet()
	...
	to_chat(body, span_danger(LANG("datum.369163e5c7abf41c", list(name))))
	to_chat(body, span_danger("[desc]"))
	switch(team)
		if(MAFIA_TEAM_MAFIA)
			to_chat(body,span_danger(LANG("datum.a1d4798e535b7a41", null)))
		if(MAFIA_TEAM_TOWN)
			to_chat(body,span_danger(LANG("datum.2520cf6bf8f2260c", null)))
		if(MAFIA_TEAM_SOLO)
			to_chat(body,span_danger(LANG("datum.ba166fcfd02bec0d", null)))
	to_chat(body, LANG("datum.dfe0a8095fecb907", null))
