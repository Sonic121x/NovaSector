	...
	if(ui.user.client.holder)
		switch(action)
			...
			if("players_home")
				...
				if(failed.len)
					to_chat(usr, LANG("datum.46a82c407521d413", null))
			...
			if("debug_setup")
				...
				while(!done)
					to_chat(usr, LANG("datum.37dbff01482beb28", list(counterlist_sum(debug_setup))))
					var/chosen_role_name = tgui_input_list(usr, LANG("datum.0070f3e32a4c633d", null), LANG("datum.5828e4e466fb9407", null), rolelist_dict)
					if(!chosen_role_name)
						return
					switch(chosen_role_name)
						...
						if("FINISH")
							...
							var/role_count = tgui_input_number(usr, LANG("datum.08ea88c7b24c936e", null), LANG("datum.5828e4e466fb9407", null), 0, 12)
							if(role_count > 0)
	...
	if(!user_role) // non-player & pre-game actions.
		switch(action)
			...
			if("vote_to_start")
				...
				if(phase != MAFIA_PHASE_SETUP)
					to_chat(usr, span_notice(LANG("datum.387473898508bca3", null)))
					return
				if(isnull(modpc))
					if(!GLOB.mafia_signup[ghost_client.ckey])
						to_chat(usr, span_notice(LANG("datum.8fe3c82bdf991abf", null)))
						return
					if(GLOB.mafia_early_votes[ghost_client.ckey])
						GLOB.mafia_early_votes -= ghost_client.ckey
						to_chat(usr, span_notice(LANG("datum.4566dee2b6ebcff9", null)))
					else
						GLOB.mafia_early_votes[ghost_client.ckey] = ghost_client
						to_chat(usr, span_notice(LANG("datum.6321218447921735", list(length(GLOB.mafia_early_votes), max(round(length(GLOB.mafia_signup + GLOB.pda_mafia_signup) / 2), round(MAFIA_MIN_PLAYER_COUNT / 2))))))
