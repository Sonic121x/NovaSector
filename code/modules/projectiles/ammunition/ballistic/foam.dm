...
/obj/item/ammo_casing/foam_dart/examine_more(mob/user)
	...
	if(!HAS_TRAIT(src, TRAIT_DART_HAS_INSERT))
		...
		. += span_notice(LANG("obj.4a7ad516", list(modified ? "You can" : "If you removed the safety cap with a screwdriver, you could", length(type_initial_names) ? ", such as [lang_english_list(type_initial_names, and_text = "or ", final_comma_text = ", ")]" : "")))
