...
/obj/item/gun/magic/hook/suicide_act(mob/living/user)
	...
	if(isnull(removable))
		user.visible_message(span_suicide("[user]将[src]的链条塞进了本该是头部的位置！看起来[user.p_theyre()]想要自杀！"))
		return OXYLOSS

	playsound(get_turf(src), fire_sound, 50, TRUE, -1)
	user.visible_message(span_suicide("[user]正将[src]对准[user.p_their()]头部！看起来[user.p_theyre()]想要自杀！"))
...
/obj/projectile/hook/on_hit(atom/target, blocked = 0, pierce_hit)
	...
	victim.visible_message(span_danger("[victim]被[firer]的钩子钩住了！"))
