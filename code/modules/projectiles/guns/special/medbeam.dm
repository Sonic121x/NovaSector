...
/obj/item/gun/medbeam/proc/on_beam_tick(mob/living/target)
	...
	var/need_mob_update
	need_mob_update = target.adjust_brute_loss(-4, updating_health = FALSE)
	need_mob_update += target.adjust_fire_loss(-4, updating_health = FALSE)
	need_mob_update += target.adjust_tox_loss(-1, updating_health = FALSE, forced = TRUE)
	need_mob_update += target.adjust_oxy_loss(-1, updating_health = FALSE)
	if(need_mob_update)
		target.updatehealth()
	return
