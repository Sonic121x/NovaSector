...
/obj/item/gun/ballistic
	name = "发射器"
	desc = "现在有多种口味，比如“枪”。不知为何使用10毫米弹药。"
	icon_state = "debug"
...
/obj/item/gun/ballistic/proc/burst_select()
	...
	if(!burst_fire_selection)
		...
		balloon_alert(user, "切换到[burst_size]发点射模式")
					...
					switch(caught_casing)
						...
						if(CASING_CATCH_FAILED_SPICY)
							...
							wielder.visible_message(
								span_warning("[wielder] 伸手去接从 [casing] 中抛出的 \the [src]，并抓住了它……然后因为弹壳太烫而失手掉了。真不酷！"),
								span_warning("你伸手接住了从 [casing] 中抛出的 \the [src]……然后因为弹壳太烫而掉了！哎哟！真不酷！"),
								span_notice("你听到有人伸手去够什么东西，接着是一声痛苦的嘶嘶声和什么东西掉落的哐当声。"),
							)
						...
						if(CASING_CATCH_FAILED_CLUMSY)
							hitting_ground = TRUE
							wielder.visible_message(
								span_warning("[wielder]伸手去接从[casing]弹出的\the [src]……结果却以一种极其不可能、滑稽的方式把它弄掉了！太逊了！"),
...
/obj/item/gun/ballistic/proc/rack(mob/user = null)
	...
	if (bolt_type == BOLT_TYPE_OPEN)
		if(!bolt_locked) //If it's an open bolt, racking again would do nothing
			if (user)
				balloon_alert(user, "[bolt_wording] 已经上膛了！")
			return
		bolt_locked = FALSE
	if (user)
		balloon_alert(user, "[bolt_wording] 已上膛")
...
/obj/item/gun/ballistic/proc/drop_bolt(mob/user = null)
	...
	if (user)
		balloon_alert(user, "[bolt_wording] 已放下")
	chamber_round()
	bolt_locked = FALSE
	update_appearance()
...
/obj/item/gun/ballistic/proc/eject_magazine(mob/user, display_message = TRUE, obj/item/ammo_box/magazine/tac_load = null)
	...
	if (tac_load)
		if (insert_magazine(user, tac_load, FALSE))
			balloon_alert(user, "[magazine_wording] 已更换")
		else
	...
	if (display_message)
		balloon_alert(user, "[magazine_wording] 已卸下")
	update_appearance()
...
/obj/item/gun/ballistic/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	...
	if(istype(tool, /obj/item/suppressor))
		if(!can_suppress)
			balloon_alert(user, "[tool.name] 不匹配！")
			return ITEM_INTERACT_FAILURE
		...
		balloon_alert(user, "[tool.name] 已安装")
...
/obj/item/gun/ballistic/process_fire(atom/target, mob/living/user, message = TRUE, params = null, zone_override = "", bonus_spread = 0)
	...
	if(target != user && chambered.loaded_projectile && could_it_misfire && prob(misfire_probability) && blow_up(user))
		to_chat(user, span_userdanger("[src] 哑火了！"))
		return
...
/obj/item/gun/ballistic/click_alt(mob/user)
	...
	if(!user.is_holding(src))
		return CLICK_ACTION_BLOCKING
	balloon_alert(user, "[suppressor.name] 已移除")
...
/obj/item/gun/ballistic/proc/unload_ammo(mob/living/user, forced = FALSE)
	...
	if (!forced)
		balloon_alert(user, "[num_unloaded] [cartridge_wording]\s 已卸下")
...
/obj/item/gun/ballistic/examine(mob/user)
	...
	if(can_misfire)
		. += span_danger("你感觉这东西开火可能会爆炸...")
		if(misfire_probability > 0)
			. += span_danger("考虑到这把枪的状况，有 [misfire_probability]% 的几率会哑火。")
	else if(misfire_probability > 0)
		. += span_warning("你感觉如果用错弹药，这东西开火可能会爆炸...")
		. += span_warning("考虑到这把枪的状况，有 [EXAMINE_HINT("[misfire_probability]%")] 的几率会哑火。")
...
/obj/item/gun/ballistic/suicide_act(mob/living/user)
	...
	if (B && chambered && chambered.loaded_projectile && can_trigger_gun(user) && chambered.loaded_projectile.damage > 0)
		user.visible_message(span_suicide("[user] 正把 [src] 的枪管塞进 [user.p_their()] 嘴里。看起来 [user.p_theyre()] 想自杀！"))
		sleep(2.5 SECONDS)
		if(user.is_holding(src))
			var/turf/T = get_turf(user)
			process_fire(user, user, FALSE, null, BODY_ZONE_HEAD)
			user.visible_message(span_suicide("[user] 用 [user.p_their()] 轰掉了 [user.p_s()] 的脑浆[src]！"))
		...
		user.visible_message(span_suicide("[user] 假装用 [user.p_their()] 轰掉 [user.p_s()] 的脑浆[src]！看起来 [user.p_theyre()] 想自杀！</b>"))
...
/obj/item/gun/ballistic/proc/sawoff(mob/user, obj/item/saw, handle_modifications = TRUE)
	...
	if(blow_up(user))
		user.visible_message(span_danger("[src] 走火了！"), span_danger("[src] 在你面前走火了！"))
		return
	...
	if(sawn_off)
		return
	user.visible_message(span_notice("[user] 截短了[src]！"), span_notice("你截短了[src]。"))
	sawn_off = TRUE
...
/obj/item/gun/ballistic/wrench_act(mob/living/user, obj/item/I)
	...
	if(!user.is_holding(src))
		balloon_alert(user, "按住以改装！")
		return TRUE

	if(get_ammo())
		balloon_alert(user, "装弹时无法改装！")
		return

	if(!bolt_locked && bolt_type == BOLT_TYPE_LOCKING)
		balloon_alert(user, "枪栓挡住了！")
		return

	balloon_alert(user, "正在改装...")
	...
	if(magazine.caliber == initial_caliber)
		...
		to_chat(user, span_notice("你改装了[src]。现在它将发射[alternative_caliber]口径的子弹。"))
	else
		magazine.caliber = initial_caliber
		if(alternative_ammo_misfires)
			can_misfire = FALSE
		fire_sound = initial_fire_sound
		to_chat(user, span_notice("你重置了[src]。现在它将发射[initial_caliber]口径的子弹。"))
