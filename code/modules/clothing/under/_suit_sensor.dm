...
/obj/item/suit_sensor
	name = "制服传感器"
	desc = "医疗部一直让你设置成‘追踪信标’的那个小玩意儿。它需要附着在穿着的制服或连衣裙上才能工作。"
...
/obj/item/suit_sensor/examine(mob/user)
	. = ..()
	if(broken)
		. += span_warning("它目前是坏的。你可以用一段[EXAMINE_HINT("cable")]来修复它。")
	else
		. += span_notice("它当前设置为'[GLOB.suit_sensor_mode_to_defines.Find(sensor_mode + 1)]'。")
...
/obj/item/suit_sensor/attack_self(mob/living/user)
	...
	if(!(user.mobility_flags & MOBILITY_USE) || !IsReachableBy(user))
		return FALSE
	if(broken)
		balloon_alert(user, "先修好它！")
		return
	var/current_mode_text = GLOB.suit_sensor_mode_to_defines[sensor_mode + 1]
	var/new_mode = tgui_input_list(user, "选择传感器模式", "制服传感器", GLOB.suit_sensor_mode_to_defines, current_mode_text)
	if(isnull(new_mode) || broken|| !(user.mobility_flags & MOBILITY_USE) || !IsReachableBy(user))
		user.balloon_alert(user, "现在不能这么做！")
		return
	set_mode(GLOB.suit_sensor_mode_to_defines[new_mode])
	balloon_alert(user, "传感器已设置为'[LOWER_TEXT(new_mode)]'")
