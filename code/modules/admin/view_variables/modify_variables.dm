...
/client/proc/vv_parse_text(O, new_var)
	if(O && findtext(new_var,"\["))
		var/process_vars = tgui_alert(usr,"\[] detected in string, process as variables?","处理变量？",list("Yes","No"))
		if(process_vars == "Yes")
...
/client/proc/mod_list(list/L, atom/O, original_name, objectvar, index, autodetect_class = FALSE)
	...
	if(L.len > 1000)
		var/confirm = tgui_alert(usr, "你正在尝试编辑的列表非常长，继续操作可能导致服务器崩溃。", "警告", list("Continue", "Abort"))
		if(confirm != "Continue")
			return
	...
	for (var/i in 1 to L.len)
		...
		names["#[i] [key] = [value]"] = i
	if (!index)
		var/variable = input("哪个变量？","变量") as null|anything in names + "(添加变量)" + "(清除空值)" + "(清除重复项)" + "(随机排序)"
	...
	if (index == null)
		return
	var/assoc = 0
	var/prompt = tgui_alert(usr, "你想要编辑键还是其分配的值？", "关联列表", list("Key", "Assigned Value", "Cancel"))
	if (prompt == "Cancel")
		return
	...
	if(default == VV_NUM)
		...
		if(dir_text)
			to_chat(usr, "如果是方向，方向是: [dir_text]", confidential = TRUE)
...
/client/proc/modify_variables(atom/O, param_var_name = null, autodetect_class = 0)
	...
	if(param_var_name)
		...
		variable = input("哪个变量？","变量") as null|anything in names
		if(!variable)
			return
	...
	if (O.vv_edit_var(variable, var_new) == FALSE)
		to_chat(src, "您的编辑被对象拒绝了。", confidential = TRUE)
		return
