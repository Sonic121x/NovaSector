...
ADMIN_VERB_AND_CONTEXT_MENU(cmd_admin_subtle_message, R_ADMIN, "Subtle Message", ADMIN_VERB_NO_DESCRIPTION, ADMIN_CATEGORY_HIDDEN, mob/target in world)
	...
	var/msg = input(user, "Message:", "向 [target.key] 发送私密消息") as text|null
	...
	target.balloon_alert(target, "你听到一个声音")
...
/client/proc/admin_headset_message(mob/target in GLOB.mob_list, sender = null)
	...
	if (!sender)
		sender = input("消息来自谁？", "发送者") as null|anything in list(RADIO_CHANNEL_CENTCOM,RADIO_CHANNEL_SYNDICATE)
		if(!sender)
			return

	message_admins("[key_name_admin(src)] has started answering [key_name_admin(target)]'s [sender] request.")
	var/input = input("请输入一条消息，通过其耳机回复 [key_name(target)]。","来自 [sender] 的传出消息", "") as text|null
	if(!input)
	...
	target.balloon_alert(target, "你听到了一个声音")
	to_chat(target, span_hear("你听到 [human_recipient ? "ears" : "radio receiver"] 中传来一阵噼啪声，随后一个声音响起。\"请稍候，等待来自 [sender == "Syndicate" ? "your benefactor" : "Central Command"] 的消息。消息如下[sender == "Syndicate" ? ", agent." : ":"] <b>[input]。</b> 消息结束。\"\""), confidential = TRUE)
...
ADMIN_VERB(cmd_admin_world_narrate, R_ADMIN, "Global Narrate", "Send a direct narration to all connected players.", ADMIN_CATEGORY_EVENTS)
	var/msg = input(user, "Message:", "输入你希望向所有人显示的文本：") as text|null
	if (!msg)
		return
	msg = user.reformat_narration(msg)
...
ADMIN_VERB_AND_CONTEXT_MENU(cmd_admin_local_narrate, R_ADMIN, "Local Narrate", ADMIN_VERB_NO_DESCRIPTION, ADMIN_CATEGORY_HIDDEN, atom/locale in world)
	var/range = input(user, "Range:", "向多少格范围内的生物叙述：", 7) as num|null
	if(!range)
		return
	var/msg = input(user, "Message:", "输入你希望向视野内所有人显示的文本：") as text|null
	if (!msg)
		return
	msg = user.reformat_narration(msg)
...
ADMIN_VERB_AND_CONTEXT_MENU(cmd_admin_direct_narrate, R_ADMIN, "Direct Narrate", ADMIN_VERB_NO_DESCRIPTION, ADMIN_CATEGORY_HIDDEN, mob/target)
	var/msg = input(user, "Message:", "输入你想让目标看到的文本：") as text|null
...
ADMIN_VERB(cmd_admin_add_freeform_ai_law, R_ADMIN, "Add Custom AI Law", "Add a custom law to the Silicons.", ADMIN_CATEGORY_EVENTS)
	var/input = input(user, "请输入任何你想让AI做的事情。任何事情。认真的。", "What?", "") as text|null
	if(!input)
		return
	...
	var/show_log = tgui_alert(user, "显示离子风暴消息？", "消息", list("Yes", "No"))
...
ADMIN_VERB(toggle_nuke, R_DEBUG|R_ADMIN, "Toggle Nuke", "Arm or disarm a nuke.", ADMIN_CATEGORY_EVENTS)
	...
	for (var/obj/machinery/nuclearbomb/bomb in world)
		nukes += bomb
	var/obj/machinery/nuclearbomb/nuke = tgui_input_list(user, "", "Toggle Nuke-切换核弹", nukes)
	if (isnull(nuke))
		return
	if(!nuke.timing)
		var/newtime = tgui_input_number(user, "设置激活计时器。", "激活核弹", nuke.timer_set)
		if(!newtime)
			return
...
ADMIN_VERB(change_sec_level, R_ADMIN, "Set Security Level", "Changes the security level. Announcement effects only.", ADMIN_CATEGORY_EVENTS)
	var/level = tgui_input_list(user, "选择安保等级：", "Set Security Level-威胁等级", SSsecurity_level.available_levels)
...
/client/proc/reformat_narration(input)
	if(tgui_alert(mob, "设置自定义文本格式？", "让它炫酷起来！", list("Yes", "No")) == "Yes")
		var/text_span = tgui_input_list(mob, "选择一个样式！", "沉浸感！耶！", GLOB.spanname_to_formatting)
