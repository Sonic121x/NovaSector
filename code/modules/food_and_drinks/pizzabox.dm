...
/obj/item/pizzabox/update_desc()
	...
	if(open)
		if(pizza)
			desc = "[desc] 里面似乎装着\a [pizza][pizza.sliced ? ". It is sliced" : ""]。用你的另一只手把它拿出来。"
		if(bomb)
			desc = "[desc] 等等，我草?! 这里面有个 \a [bomb]！"
			if(bomb_defused)
				desc = "[desc] 炸弹看起来并没有启动. 用你的另一只手来调控它"
			if(bomb_active)
				desc = "[desc] 这玩意要炸了，你还在看什么！？"
	else
		var/obj/item/pizzabox/box = length(boxes) ? boxes[length(boxes)] : src
		if(length(boxes))
			desc = "一堆适合装披萨的盒子。看起来这堆里有[length(boxes) + 1]个盒子。"
		if(box.boxtag != "")
			desc = "[desc] [length(boxes) ? "top box" : "box"]的标签上写着：[box.boxtag]。"
...
/obj/item/pizzabox/attack_self(mob/user)
	...
	if(open && !bomb_defused)
		audible_message(span_warning("[icon2html(src, hearers(src))] *哔*"))
		bomb_active = TRUE
	...
	if(open)
		if(pizza)
			...
			if(wires.is_all_cut() && bomb_defused)
				...
				balloon_alert(user, "移除了炸弹")
				clear_bomb()
				update_appearance()
				return
			else
				bomb_timer = tgui_input_number(user, "设置炸弹计时器", "披萨炸弹", bomb_timer, bomb_timer_max, bomb_timer_min)
...
/obj/item/pizzabox/item_interaction(mob/living/user, obj/item/used_item, list/modifiers)
	...
	if(istype(used_item, /obj/item/pizzabox))
		...
		if(!user.can_write(used_item))
			return ITEM_INTERACT_FAILURE
		var/obj/item/pizzabox/box = length(boxes) ? boxes[length(boxes)] : src
		box.boxtag += tgui_input_text(user, "在[box]的标签上写下：", box, max_length = 30)
		if(!user.can_perform_action(src))
			return ITEM_INTERACT_FAILURE
		balloon_alert(user, "正在写盒子标签...")
...
/obj/item/pizzabox/examine(mob/user)
	...
	if(isobserver(user))
		if(bomb)
			. += span_deadsay("这个披萨盒里装着[bomb_defused ? "an unarmed bomb" : "an armed bomb"]。")
		if(pizza && istype(pizza, /obj/item/food/pizza/margherita/robo))
			. += span_deadsay("这个披萨盒里的披萨含有纳米机器。")

/obj/item/pizzabox/proc/disperse_pizzas()
	visible_message(span_warning("披萨散落得到处都是！"))
	for(var/V in boxes)
...
/obj/item/pizzabox/infinite/examine(mob/user)
	...
	. = ..()
	if(isobserver(user))
		. += span_deadsay("这个披萨盒是异常的，能产生无限的披萨。")

/obj/item/pizzabox/infinite/attack_self(mob/living/user)
	if(ishuman(user))
		attune_pizza(user)
		to_chat(user, span_notice("盒子里立刻又出现了一个披萨，这到底是怎么回事？"))
	return ..()
