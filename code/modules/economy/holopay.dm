...
/obj/structure/holopay/examine(mob/user)
	. = ..()
	if(force_fee)
		. += span_boldnotice("此全息支付器强制每次刷卡支付<b>[force_fee]</b> [MONEY_NAME_AUTOPURAL(force_fee)]，而非可变金额。")
...
/obj/structure/holopay/attackby(obj/item/held_item, mob/item_holder, list/modifiers, list/attack_modifiers)
	...
	if(isidcard(held_item))
		if(istype(held_item, /obj/item/card/id/departmental_budget))
			balloon_alert(user, "无效的支付卡")
			to_chat(user, span_warning("您不能为此使用部门卡。"))
			return FALSE
		if(force_fee && tgui_alert(item_holder, "此全息支付终端收取 [force_fee] [MONEY_SYMBOL] 费用。确认？", "全息支付终端费用", list("Pay", "Cancel")) != "Pay")
			return TRUE
	...
	if(istype(held_item, /obj/item/holochip))
		...
		if(!chip.credits)
			balloon_alert(user, "全息芯片是空的")
			to_chat(user, span_warning("这里似乎没有任何[MONEY_NAME]。"))
			return FALSE
		/// Charges force fee or uses pay what you want
		var/cash_deposit = force_fee || tgui_input_number(user, "支付多少？（最多：[chip.credits]）", "支付", max_value = chip.credits)
		/// Exit sanity checks
		if(!cash_deposit)
			return TRUE
		...
		if(!chip.spend(cash_deposit, FALSE))
			balloon_alert(user, "[MONEY_NAME] 不足")
			to_chat(user, span_warning("你的这张芯片没有足够的[MONEY_NAME]来支付。"))
			return FALSE
		/// Success: Alert buyer
	...
	if(istype(held_item, /obj/item/stack/spacecash))
		to_chat(user, "这是什么，2000年代吗？我们这里只收卡。")
		return TRUE
	if(istype(held_item, /obj/item/coin))
		to_chat(user, "这是什么，1800年代吗？我们这里只收卡。")
		return TRUE
	return ..()
...
/obj/structure/holopay/proc/dissipate()
	...
	visible_message(span_notice("支付台消失了。"))
	qdel(src)
...
/obj/structure/holopay/proc/process_payment(mob/living/user)
	...
	if(isnull(id_card) || id_card.can_be_used_in_payment(user))
		balloon_alert(user, "无效账户")
		to_chat(user, span_warning("你没有有效的账户。"))
		return FALSE
	var/datum/bank_account/payee = id_card.registered_account
	if(payee == linked_card?.registered_account)
		balloon_alert(user, "交易无效")
		to_chat(user, span_warning("你不能支付给自己。"))
		return FALSE
	if(istype(id_card, /obj/item/card/id/departmental_budget))
		balloon_alert(user, "支付卡无效")
		to_chat(user, span_warning("你不能使用部门卡进行此操作。"))
		return FALSE
	/// If the user has enough money, ask them the amount or charge the force fee
	var/amount = force_fee || tgui_input_number(user, "支付多少？(上限：[payee.account_balance])", "赞助", max_value = payee.account_balance)
	...
	if(!payee.adjust_money(-amount, "Holopay: [capitalize(name)]"))
		balloon_alert(user, "[MONEY_NAME] 不足")
		to_chat(user, span_warning("你没有足够的钱来支付这个。"))
		return FALSE
