...
/obj/machinery/power/apc/proc/fork_outlet_act(mob/living/user, obj/item/tool)
	...
	if(shock_source && metal && (panel_open || opened)) //Now you're cooking with electricity
		...
		user.visible_message(span_notice("[user.name]将[tool]塞进[src]的内部组件，爆发出阵阵火花！"))
...
/obj/machinery/power/apc/proc/cell_act(mob/living/user, obj/item/stock_parts/power_store/new_cell)
	...
	if(machine_stat & MAINT)
		balloon_alert(user, "没有电池连接器！")
		return ITEM_INTERACT_BLOCKING
	if(!user.transferItemToLoc(new_cell, src))
		return ITEM_INTERACT_BLOCKING
	cell = new_cell
	user.visible_message(span_notice("[user.name]将电池插入[src.name]！"))
...
/obj/machinery/power/apc/proc/can_place_terminal(mob/living/user, obj/item/stack/cable_coil/installing_cable, silent = TRUE)
	...
	if(host_turf.underfloor_accessibility < UNDERFLOOR_INTERACTABLE)
		if(!silent && user)
			balloon_alert(user, "移除地板板材！")
		return FALSE
	if(!isnull(terminal))
		if(!silent && user)
			balloon_alert(user, "已经布线了！")
		return FALSE
	if(!has_electronics)
		if(!silent && user)
			balloon_alert(user, "没有电路板可以布线！")
		return FALSE
	if(panel_open)
		if(!silent && user)
			balloon_alert(user, "电线妨碍放置终端！")
		return FALSE
...
/obj/machinery/power/apc/proc/cable_act(mob/living/user, obj/item/stack/cable_coil/installing_cable, is_right_clicking)
	...
	if(is_right_clicking)
		var/choice = tgui_input_list(user, "选择电源输入电缆层", "选择电缆层", GLOB.cable_name_to_layer)
		if(isnull(choice) \
	...
	user.visible_message(span_notice("[user.name]开始向APC框架添加电缆。"))
	...
	user.visible_message(span_notice("[user.name]向APC框架添加了电缆。"))
	balloon_alert(user, "已添加线缆")
...
/obj/machinery/power/apc/proc/electronics_act(mob/living/user, obj/item/electronics/apc/installing_board)
	...
	if(has_electronics)
		balloon_alert(user, "已经有一块电路板了！")
		return ITEM_INTERACT_BLOCKING

	if(machine_stat & BROKEN)
		balloon_alert(user, "框架已损坏！")
		return ITEM_INTERACT_BLOCKING

	user.visible_message(span_notice("[user.name]将电源控制板插入[src]。"))
	balloon_alert(user, "正在插入电路板...")
...
/obj/machinery/power/apc/proc/pseudocircuit_act(mob/living/user, obj/item/electroadaptive_pseudocircuit/pseudocircuit)
	if(!has_electronics)
		if(machine_stat & BROKEN)
			balloon_alert(user, "框架损坏太严重了！")
			return ITEM_INTERACT_BLOCKING
		if(!pseudocircuit.adapt_circuit(user, circuit_cost = 0.05 * STANDARD_CELL_CHARGE))
			return ITEM_INTERACT_BLOCKING
		user.visible_message(
			span_notice("[user] 制造了一块电路板并将其放入 [src]。"),
			span_notice("你调整了一块电源控制板，并将其卡入 [src] 的内部。"),
		)
		has_electronics = APC_ELECTRONICS_INSTALLED
		locked = FALSE
		return ITEM_INTERACT_SUCCESS
	...
	if(!cell)
		...
		cell = bad_cell
		user.visible_message(
			span_notice("[user] 制造了一块弱效电源电池并将其放入 [src]。"),
			span_warning("你的 [pseudocircuit.name] 在制造弱效电源电池并将其放入 [src] 时发出吃力的嗡鸣！"),
		)
		update_appearance()
		return ITEM_INTERACT_SUCCESS
...
/obj/machinery/power/apc/proc/wallframe_act(mob/living/user, obj/item/wallframe/apc/wallframe)
	...
	if((machine_stat & BROKEN) && opened == APC_COVER_REMOVED && has_electronics && terminal) // Cover is the only thing broken, we do not need to remove elctronicks to replace cover
		user.visible_message(span_notice("[user.name] 更换了缺失的 APC 外壳。"))
		balloon_alert(user, "正在更换APC外壳...")
		if(!do_after(user, 2 SECONDS, target = src)) // replacing cover is quicker than replacing whole frame
			return ITEM_INTERACT_BLOCKING
		balloon_alert(user, "外壳已更换")
		qdel(wallframe)
		...
		return ITEM_INTERACT_SUCCESS
	if(has_electronics)
		balloon_alert(user, "先移除内部的电路板！")
		return ITEM_INTERACT_BLOCKING
	user.visible_message(span_notice("[user.name] 用新的框架替换了损坏的 APC 框架。"))
	balloon_alert(user, "正在更换损坏的框架...")
	if(!do_after(user, 5 SECONDS, target = src))
		return ITEM_INTERACT_BLOCKING
	balloon_alert(user, "框架已更换")
	qdel(wallframe)
...
/obj/machinery/power/apc/crowbar_act(mob/user, obj/item/crowbar)
	...
	if((opened == APC_COVER_CLOSED || opened == APC_COVER_OPENED) && (machine_stat & BROKEN))
		...
		balloon_alert(user, "正在撬开...")
		if(!crowbar.use_tool(src, user, 5 SECONDS))
			return
		opened = APC_COVER_REMOVED
		balloon_alert(user, "外壳已移除")
		update_appearance()
		return
	...
	if((!opened && opened != APC_COVER_REMOVED) && !(machine_stat & BROKEN))
		if(coverlocked && !(machine_stat & MAINT)) // locked...
			balloon_alert(user, "外壳已锁定！")
			return
		else if(panel_open)
			balloon_alert(user, "电线阻止了打开！")
			return
		else
			opened = APC_COVER_OPENED
			update_appearance()
	...
	if(machine_stat & BROKEN)
		user.visible_message(span_notice("[user.name] 弄坏了 [name] 里面的电源控制板！"), \
			span_hear("你听到一声碎裂声。"))
		balloon_alert(user, "烧焦的电路板碎裂了")
		return
	else if(obj_flags & EMAGGED)
		obj_flags &= ~EMAGGED
		user.visible_message(span_notice("[user.name] 从 [name] 中丢弃了一块被破解的电源控制板！"))
		balloon_alert(user, "已丢弃被电磁脉冲干扰的电路板")
		return
	else if(malfhack)
		user.visible_message(span_notice("[user.name] 从 [name] 中丢弃了一块编程异常的电源控制板！"))
		balloon_alert(user, "已丢弃被重新编程的电路板")
		malfai = null
		malfhack = 0
		return
	user.visible_message(span_notice("[user.name] 从 [name] 中取出了电源控制板！"))
	balloon_alert(user, "已移除电路板")
...
/obj/machinery/power/apc/screwdriver_act(mob/living/user, obj/item/W)
	...
	if(!opened)
		if(obj_flags & EMAGGED)
			balloon_alert(user, "界面已损坏！")
			return
		toggle_panel_open()
		balloon_alert(user, "电线[panel_open ? "exposed" : "unexposed"]")
...
/obj/machinery/power/apc/welder_act(mob/living/user, obj/item/welder)
	...
	if((atom_integrity < max_integrity) && has_electronics)
		if(opened == APC_COVER_REMOVED)
			balloon_alert(user, "没有外壳可修复！")
			return
		if (machine_stat & BROKEN)
			balloon_alert(user, "损坏过重，无法修复！")
			return
	...
	if(!welder.tool_start_check(user, amount=1))
		return
	user.visible_message(span_notice("[user.name] 焊接了 [src]。"), \
						span_hear("你听到了焊接声。"))
	balloon_alert(user, "正在焊接 APC 框架")
	if(!welder.use_tool(src, user, 50, volume=50))
		return
	if((machine_stat & BROKEN) || opened == APC_COVER_REMOVED)
		new /obj/item/stack/sheet/iron(loc)
		user.visible_message(span_notice("[user.name] 用 [welder] 将 [src] 切割开来。"))
		user.balloon_alert(user, "拆解了损坏的框架")
	else
		new /obj/item/wallframe/apc(loc)
		user.visible_message(span_notice("[user.name] 用 [welder] 将 [src] 从墙上切割下来。"))
		user.balloon_alert(user, "从墙上切下了框架")
	qdel(src)
	return TRUE
...
/obj/machinery/power/apc/rcd_vals(mob/user, obj/item/construction/rcd/the_rcd)
	...
	if(!cell)
		if(machine_stat & MAINT)
			balloon_alert(user, "没有安装控制板，无法放置电池！")
			return FALSE
		return list("delay" = 5 SECONDS, "cost" = 10)

	balloon_alert(user, "已装有控制板和电池！")
	return FALSE
...
/obj/machinery/power/apc/proc/togglelock(mob/living/user)
	if(obj_flags & EMAGGED)
		...
		if(allowed(usr) && !wires.is_cut(WIRE_IDSCAN) && ((!malfhack && !remote_control_user) || (malfhack && (malfai == user || (user in malfai.connected_robots)))))
			locked = !locked
			balloon_alert(user, locked ? "已锁定" : "已解锁")
			update_appearance()
		else
