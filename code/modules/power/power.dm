...
/obj/machinery/power/examine(mob/user)
	...
	if(can_change_cable_layer)
		if(!QDELETED(powernet))
			. += span_notice("它正运行在[LOWER_TEXT(GLOB.cable_layer_to_name["[cable_layer]"])]上。")
		else
			. += span_warning("它已从[LOWER_TEXT(GLOB.cable_layer_to_name["[cable_layer]"])]断开连接。")
		. += span_notice("它的电源线可以用[EXAMINE_HINT("multitool")]来更改。")
...
/obj/machinery/power/proc/cable_layer_act(mob/living/user, obj/item/tool)
	var/choice = tgui_input_list(user, "选择运行电源线", "选择电缆层", GLOB.cable_name_to_layer)
	...
	balloon_alert(user, "现在运行在[choice]上")
