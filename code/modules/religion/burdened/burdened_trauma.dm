...
/datum/brain_trauma/special/burdened/proc/update_burden(increase, silent = FALSE)
	...
	switch(burden_level)
		if(0)
			if (!silent)
				to_chat(owner, span_warning(LANG("datum.29eacac7f9ef684e", list(GLOB.deity))))
		if(1)
			if (!silent)
				if(increase)
					to_chat(owner, span_notice(LANG("datum.915a17f53c254dfe", list(GLOB.deity))))
				else
					to_chat(owner, span_warning(LANG("datum.a3e63df85fea4812", list(GLOB.deity))))
		if(2)
			if(increase)
				if (!silent)
					to_chat(owner, span_notice(LANG("datum.300b1f72dbe20098", list(GLOB.deity))))
			else
				if (!silent)
					to_chat(owner, span_warning(LANG("datum.afe36ae78e32626c", null)))
		...
		if(3)
			if(increase)
				if (!silent)
					to_chat(owner, span_notice(LANG("datum.ecb8575ae930a453", list(GLOB.deity))))
				...
				if (!silent)
					to_chat(owner, span_warning(LANG("datum.5e10b7fa41e08090", null)))
		if(4)
			if (!silent)
				if(increase)
					to_chat(owner, span_notice(LANG("datum.c930b66de9e5c9f9", list(GLOB.deity))))
				else
					to_chat(owner, span_warning(LANG("datum.c40b43bfcfb5d1c3", null)))
		if(5)
			if(increase)
				if (!silent)
					to_chat(owner, span_notice(LANG("datum.0d9053bfa44de6df", null)))
			else
				if (!silent)
					to_chat(owner, span_warning(LANG("datum.afe36ae78e32626c", null)))
		...
		if(6)
			if(increase)
				if (!silent)
					to_chat(owner, span_notice(LANG("datum.8710704c0dd69d4b", null)))
		...
		if(7)
			if (!silent)
				if(increase)
					to_chat(owner, span_notice(LANG("datum.a17b135ba8c62cff", list(GLOB.deity))))
				else
					to_chat(owner, span_warning(LANG("datum.c40b43bfcfb5d1c3", null)))
		if(8)
			if (!silent)
				if(increase)
					to_chat(owner, span_notice(LANG("datum.fdcce58b500767a0", list(GLOB.deity))))
				else
					to_chat(owner, span_warning(LANG("datum.afe36ae78e32626c", null)))
		if(9)
			if(increase)
				if (!silent)
					to_chat(owner, span_notice(LANG("datum.69dd13a0b2e4dcda", list(GLOB.deity))))
