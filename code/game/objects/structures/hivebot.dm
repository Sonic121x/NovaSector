...
/obj/structure/hivebot_beacon/Initialize(mapload)
	. = ..()
	do_smoke(2, src, loc)
	visible_message(span_bolddanger(LANG("obj.f94e2e40d3fe9272", list(src))))
...
/obj/structure/hivebot_beacon/proc/warpbots()
	...
	visible_message(span_danger(LANG("obj.05f621b31bbaa716", list(src))))
	while(bot_amt > 0)
		bot_amt--
		switch(bot_type)
			if("norm")
	...
	sleep(10 SECONDS)
	visible_message(span_bolddanger(LANG("obj.26fe3bf79d7dccda", list(src))))
	playsound(src.loc, 'sound/effects/empulse.ogg', 25, TRUE)
	qdel(src)
	return
