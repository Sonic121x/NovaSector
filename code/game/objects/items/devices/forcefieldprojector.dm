...
/obj/item/forcefield_projector/ranged_interact_with_atom(atom/interacting_with, mob/living/user, list/modifiers)
	...
	if(istype(interacting_with, /obj/structure/projected_forcefield))
		...
		if(F.generator == src)
			to_chat(user, span_notice("你关闭了[F]。"))
			qdel(F)
	...
	if(get_dist(T,src) > field_distance_limit)
		return ITEM_INTERACT_BLOCKING
	if(get_turf(src) == T)
		to_chat(user, span_warning("目标太近了，中止！"))
		return ITEM_INTERACT_BLOCKING
	if(LAZYLEN(current_fields) >= max_fields)
		to_chat(user, span_warning("[src] 无法维持更多力场了！"))
		return ITEM_INTERACT_BLOCKING
	if(force_proj_busy)
		to_chat(user, span_notice("[src] 正在生成力场。"))
		return ITEM_INTERACT_BLOCKING
	...
	user.visible_message(span_warning("[user] 投射出一个力场！"),span_notice("你投射出一个力场。"))
...
/obj/item/forcefield_projector/attack_self(mob/user)
	if(LAZYLEN(current_fields))
		to_chat(user, span_notice("你关闭了 [src]，停用了所有活跃的力场。"))
		for(var/obj/structure/projected_forcefield/F in current_fields)
			qdel(F)

/obj/item/forcefield_projector/examine(mob/user)
	. = ..()
	. += span_notice("它当前维持着 [LAZYLEN(current_fields)]/[max_fields] 个力场，并且充能度为 [round((shield_integrity/max_shield_integrity)*100)]%。")
...
/obj/structure/projected_forcefield
	name = "力场"
	desc = "一道发光的屏障，由附近的投射器生成。如果受到足够多次攻击，它可能会过载。"
...
/obj/structure/projected_forcefield/Destroy()
	visible_message(span_warning("[src] 闪烁了一下并消失了！"))
