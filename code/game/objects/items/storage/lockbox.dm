...
/obj/item/storage/lockbox/emag_act(mob/user, obj/item/card/emag/emag_card)
	if(!broken)
		...
		if (emag_card && user)
			user.visible_message(span_warning(LANG("obj.3c26167e7115e3e0", list(user, emag_card, src))))
		return TRUE
	return FALSE
...
/obj/item/storage/lockbox/medal/examine(mob/user)
	...
	if(!atom_storage.locked)
		. += span_notice(LANG("obj.b66c3082524bf3a8", list(open ? "close":"open")))
...
/obj/item/storage/lockbox/order/can_unlock(mob/living/user, obj/item/card/id/id_card, silent = FALSE)
	...
	//NOVA EDIT ADDITION END
	if(!silent)
		balloon_alert(user, LANG("obj.f38773c9884dc747", null))
	return FALSE
