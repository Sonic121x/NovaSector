...
/obj/item/hand_tele/proc/try_dispel_portal(atom/target, mob/user)
	if(is_parent_of_portal(target))
		to_chat(user, span_notice(LANG("obj.68de5563e79c1ddc", list(target, src))))
...
/obj/item/hand_tele/interact_with_atom_secondary(atom/interacting_with, mob/living/user, list/modifiers)
	...
	if (isnull(portal_location))
		to_chat(user, span_warning(LANG("obj.8190e22eca100d62", list(src))))
...
/obj/item/hand_tele/attack_self(mob/user)
	...
	var/teleport_location_key = tgui_input_list(user, LANG("obj.46030f634d462199", null), LANG("obj.40db05ecce5bbc0b", null), sort_list(locations))
...
/obj/item/hand_tele/proc/try_create_portal_to(mob/user, teleport_location)
	...
	if (teleport_target == null)
		to_chat(user, span_notice(LANG("obj.29f93e16134db4d8", list(src))))
		return

	if(!check_teleport_valid(src, teleport_target))
		to_chat(user, span_notice(LANG("obj.654883d6251ffbad", list(src))))
		return
	...
	if(QDELETED(portal1) || QDELETED(portal2)) //in the event that something managed to delete the portal objects, i.e. something teleported them
		to_chat(user, span_notice(LANG("obj.617436a9fbefbf02", list(src))))
		return
...
/obj/item/hand_tele/suicide_act(mob/living/user)
	if(iscarbon(user))
		user.visible_message(span_suicide(LANG("obj.01a413fbd08fe63a", list(user, user.p_their(), user.p_theyre()))))
		...
		if(head)
			...
			itemUser.visible_message(span_suicide(LANG("obj.27851e33c58ab589", list(user))))
		else
			itemUser.visible_message(span_suicide(LANG("obj.e6aad8b5a46e3f81", list(user))))
		return BRUTELOSS
...
/obj/item/syndicate_teleporter/examine(mob/user)
	. = ..()
	. += span_notice(LANG("obj.13be2784f6b152c0", list(src, charges, max_charges)))
...
/obj/item/syndicate_teleporter/process(seconds_per_tick)
	if(SPT_PROB(10, seconds_per_tick) && charges < max_charges)
		...
		if(ishuman(loc))
			...
			balloon_alert(holder, LANG("obj.4f21efa71d857d81", null))
...
/obj/item/syndicate_teleporter/emp_act(severity)
	...
	if(ishuman(loc))
		var/mob/living/carbon/human/holder = loc
		balloon_alert(holder, LANG("obj.30cbfdbc41944984", null))
		...
		if(!teleported_something)
			visible_message(span_danger(LANG("obj.9c76d0dfa0049c0c", list(src))))
			do_sparks(2, 1, src)
			qdel(src)
	...
	if(isclosedturf(destination))
		if(!triggered_by_emp && !bagholdingcheck)
			...
			if(bagholdingcheck && !not_holding_tele)
				to_chat(user, span_warning(LANG("obj.7393b48106577dea", null)))
...
/obj/item/syndicate_teleporter/proc/panic_teleport(mob/user, turf/destination)
	...
	if(emergency_destination)
		...
		balloon_alert(user, LANG("obj.a0e5bd228b38e8a9", null))
...
/obj/item/syndicate_teleporter/proc/get_fragged(mob/living/victim, turf/destination, not_holding_tele = FALSE)
	...
	if(!not_holding_tele)
		to_chat(victim, span_userdanger(LANG("obj.e5873e5ca3d7139e", list(destination, src))))
	else
		to_chat(victim, span_userdanger(LANG("obj.eab3be64be60fecb", list(destination))))
...
/obj/item/syndicate_teleporter/proc/telefrag(turf/fragging_location, mob/user) // Don't let this gib. Never let this gib.
	for(var/mob/living/victim in fragging_location)//Hit everything in the turf
		...
		to_chat(victim, span_warning(LANG("obj.6f3768200afb0bd3", list(user))))
...
/obj/item/syndicate_teleporter/proc/make_bloods(turf/old_location, turf/new_location, mob/living/user)
	...
	if(prob(25) && bleed_amount)
		...
		visible_message(span_warning(LANG("obj.da731dc00c1728e3", list(user, src, user.p_their()))), \
			span_boldwarning(LANG("obj.ce932b8b3fda37a5", list(src))))
