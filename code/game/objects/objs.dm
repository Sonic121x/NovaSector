...
/obj/attacked_by(obj/item/attacking_item, mob/living/user, list/modifiers, list/attack_modifiers)
	...
	if(!LAZYACCESS(attack_modifiers, SILENCE_DEFAULT_MESSAGES))
		...
		user.visible_message(
			span_danger(LANG("obj.deff8f6f48be9174", list(user, message_verb_continuous, src, attacking_item, damage ? "." : ", [no_damage_feedback]!"))),
			span_danger(LANG("obj.93c74fc119960f01", list(message_verb_simple, src, attacking_item, damage ? "." : ", [no_damage_feedback]!"))),
			null,
...
/obj/vv_do_topic(list/href_list)
	...
	if(href_list[VV_HK_MASS_DEL_TYPE])
		if(!check_rights(R_DEBUG|R_SERVER))
			return
		var/action_type = tgui_alert(usr, LANG("obj.68e254d380f2e571", list(type)),,list("Strict type","Type and subtypes","Cancel"))
		if(action_type == "Cancel" || !action_type)
			return
		if(tgui_alert(usr, LANG("obj.c71c86972d5581ab", list(type)),,list("Yes","No")) != "Yes")
			return
		if(tgui_alert(usr, LANG("obj.bd025aa5cea64e01", null),,list("Yes","No")) != "Yes")
			return
		var/O_type = type
		switch(action_type)
			if("Strict type")
				...
				for(var/obj/Obj in world)
					...
					CHECK_TICK
				if(!i)
					to_chat(usr, LANG("obj.d6c5da78fc2545e9", null))
					return
...
/obj/proc/can_be_unfasten_wrench(mob/user, silent)
	if(!is_anchorable_floor(loc) && !anchored)
		if(!silent)
			to_chat(user, span_warning(LANG("obj.ec1e097463cb05cc", list(src))))
...
/obj/proc/default_unfasten_wrench(mob/user, obj/item/wrench, time = 2 SECONDS)
	...
	if(!anchored && ground.is_blocked_turf(exclude_mobs = TRUE, source_atom = src))
		to_chat(user, span_notice(LANG("obj.8588de2d5a426f84", list(src))))
		return CANT_UNFASTEN
	var/can_be_unfasten = can_be_unfasten_wrench(user)
	if(!can_be_unfasten || can_be_unfasten == FAILED_UNFASTEN)
		return can_be_unfasten
	if(time)
		to_chat(user, span_notice(LANG("obj.da583a84b6d61157", list(anchored ? "un" : "", src))))
