...
/obj/machinery/recharger/examine(mob/user)
	...
	if(!in_range(user, src) && !issilicon(user) && !isobserver(user))
		. += span_warning("您距离太远，无法查看[src]的内容并显示!")
		return
	...
	if(charging_cell)
		. += span_notice("- \The [charging] 的电池电量为 <b>[charging_cell.percent()]%</b>。")
		return
	if(istype(charging, /obj/item/ammo_box/magazine/recharge))
		var/obj/item/ammo_box/magazine/recharge/power_pack = charging
		. += span_notice("- \The [charging] 的电池电量为 <b>[PERCENT(power_pack.stored_ammo.len/power_pack.max_ammo)]%</b>。")
		return
	if(istype(charging, /obj/item/gun/ballistic/automatic/battle_rifle))
		...
		. += span_notice("- \The [charging] 的系统退化处于第 [recalibrating_gun.degradation_stage] 阶段，共 [recalibrating_gun.degradation_stage_max] 阶段</b>。")
		. += span_notice("- \The [charging] 的退化缓冲为 <b>[PERCENT(recalibrating_gun.shots_before_degradation/recalibrating_gun.max_shots_before_degradation)]%</b>。")
		return
	. += span_notice("- \The [charging] 没有报告电量。")
...
/obj/machinery/recharger/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	...
	if(!anchored)
		to_chat(user, span_notice("[src] 并未和任何东西相连!"))
	...
	if(istype(tool, /obj/item/gun/energy))
		...
		if(!energy_gun.can_charge)
			to_chat(user, span_notice("您的枪没有外部电源接口."))
			return ITEM_INTERACT_BLOCKING
...
/obj/machinery/recharger/wrench_act(mob/living/user, obj/item/tool)
	if(charging)
		to_chat(user, span_notice("首先拿走充电项目!"))
		return ITEM_INTERACT_BLOCKING
	set_anchored(!anchored)
	power_change()
	to_chat(user, span_notice("你 [anchored ? "attached" : "detached"] [src]."))
