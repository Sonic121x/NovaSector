...
/obj/machinery/firealarm/emag_act(mob/user, obj/item/card/emag/emag_card)
	...
	if(user)
		balloon_alert(user, LANG("obj.ae8d6631bc37ef97", null))
...
/obj/machinery/firealarm/proc/alarm(mob/user, silent = FALSE)
	...
	if(user)
		if(!silent)
			balloon_alert(user, LANG("obj.3b397ade10f2f01b", null))
...
/obj/machinery/firealarm/proc/reset(mob/user, silent = FALSE)
	...
	if(user)
		if(!silent)
			balloon_alert(user, LANG("obj.6ab5a6c048c9e421", null))
...
/obj/machinery/firealarm/screwdriver_act(mob/living/user, obj/item/tool)
	...
	balloon_alert_to_viewers(LANG("obj.ac8966cf0cdb5091", list(panel_open ? "exposed" : "unexposed")))
	update_appearance()
...
/obj/machinery/firealarm/welder_act(mob/living/user, obj/item/tool)
	...
	if(atom_integrity >= max_integrity)
		balloon_alert(user, LANG("obj.43379d5f8d288697", null))
		return ITEM_INTERACT_BLOCKING
	if(!tool.tool_start_check(user, amount = 1))
		return ITEM_INTERACT_BLOCKING
	balloon_alert_to_viewers(LANG("obj.b52342a8e93a2ba2", null))
...
/obj/machinery/firealarm/wirecutter_act_secondary(mob/living/user, obj/item/tool)
	if(!panel_open)
		return NONE
	if(buildstage != FIRE_ALARM_BUILD_SECURED)
		balloon_alert(user, LANG("obj.7b3a589a3676288f", null))
		return ITEM_INTERACT_BLOCKING

	tool.play_tool_sound(src)
	new /obj/item/stack/cable_coil(user.loc, 5)
	balloon_alert_to_viewers(LANG("obj.5191243ca52ca7ce", null))
...
/obj/machinery/firealarm/crowbar_act(mob/living/user, obj/item/tool)
	...
	loc.balloon_alert_to_viewers(LANG("obj.c213b154ab7d685c", null))
	if(!tool.use_tool(src, user, 2 SECONDS, volume = 50, extra_checks = CALLBACK(src, PROC_REF(state_callback), FIRE_ALARM_BUILD_NO_WIRES, TRUE)))
		return ITEM_INTERACT_BLOCKING
	if(machine_stat & BROKEN)
		balloon_alert_to_viewers(LANG("obj.abf8870dc1ed7921", null))
		set_machine_stat(machine_stat & ~BROKEN)
	else
		balloon_alert_to_viewers(LANG("obj.6bf68b4883ef1d50", null))
...
/obj/machinery/firealarm/wrench_act(mob/living/user, obj/item/tool)
	...
	if(buildstage != FIRE_ALARM_BUILD_NO_CIRCUIT)
		balloon_alert(user, LANG("obj.34f3b9b29db3bc82", list(buildstage == FIRE_ALARM_BUILD_SECURED ? "wires" : "circuit")))
		return ITEM_INTERACT_BLOCKING

	loc.balloon_alert_to_viewers(LANG("obj.2a4235b4837591d1", list(/obj/item/wallframe/firealarm::name)))
...
/obj/machinery/firealarm/tool_act(mob/living/user, obj/item/tool, list/modifiers)
	...
	if(!is_wire_tool(tool))
		return NONE
	if(!panel_open)
		balloon_alert(user, LANG("obj.21a6eee52d5b807e", null))
		return ITEM_INTERACT_BLOCKING
	wires.interact(user)
	return ITEM_INTERACT_SUCCESS
...
/obj/machinery/firealarm/proc/cable_act(mob/living/user, obj/item/stack/cable_coil/coil)
	if(buildstage != FIRE_ALARM_BUILD_NO_WIRES)
		return NONE
	if(!coil.use(5))
		balloon_alert(user, LANG("obj.bb6f7f63453c3a85", null))
		return ITEM_INTERACT_BLOCKING

	balloon_alert_to_viewers(LANG("obj.7eb1b62313424bf0", null))
...
/obj/machinery/firealarm/proc/electronics_act(mob/living/user, obj/item/electronics/firealarm/circuit)
	...
	balloon_alert_to_viewers(LANG("obj.3bf49b8db1d12d4f", null))
	qdel(circuit)
	buildstage = FIRE_ALARM_BUILD_NO_WIRES
	update_appearance()
	return ITEM_INTERACT_SUCCESS
...
/obj/machinery/firealarm/examine(mob/user)
	...
	if((my_area?.fire || LAZYLEN(my_area?.active_firelocks)))
		. += LANG("obj.3ad9a286d4fb6cb4", null)
		. += LANG("obj.d05e0bfee4bc3ca8", list(my_area.fault_location, my_area.fault_status == AREA_FAULT_AUTOMATIC ? "Automatic Detection" : "Manual Trigger"))
		if(is_station_level(z))
			. += LANG("obj.e5a011c7128a2bf4", list(SSsecurity_level.get_current_level_as_text()))
		. += LANG("obj.8591689390fa3a74", null)
		. += LANG("obj.9d65a459a5be173d", null)
	else
		if(is_station_level(z))
			. += LANG("obj.e5a011c7128a2bf4", list(SSsecurity_level.get_current_level_as_text()))
		. += LANG("obj.70b85ec41b315cea", list(my_area.fire_detect ? "lit" : "unlit"))
		. += LANG("obj.8591689390fa3a74", null)
...
/obj/machinery/firealarm/AICtrlClick(mob/living/silicon/robot/user)
	if(obj_flags & EMAGGED)
		balloon_alert(user, LANG("obj.42f4edf6bcdc22c6", null))
		return
...
/obj/machinery/firealarm/proc/toggle_fire_detect(mob/user, silent = FALSE)
	if(!can_toggle_detection)
		if(user && !silent)
			balloon_alert(user, LANG("obj.5d62aaa04dc423e8", null))
		return
	...
	if (user && !silent)
		balloon_alert(user, LANG("obj.4680a2cd5ffb503d", list(my_area.fire_detect ? "enabled" : "disabled")))
