...
/obj/machinery/computer/warrant/proc/print_bounty(mob/user, list/params)
	...
	printing = TRUE
	balloon_alert(user, "打印中")
	playsound(src, 'sound/machines/printer.ogg', 100, TRUE)

	var/obj/item/paper/bounty = new(null)
	bounty.name = "[target.name]的悬赏令"
	bounty.desc = "一份针对[target.name]的[warrant.fine][MONEY_SYMBOL]悬赏。"
